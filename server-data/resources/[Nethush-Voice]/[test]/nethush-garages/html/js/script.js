...
$(document).on('click', '.close-menu > i', function(e) {
  e.preventDefault();
  $.post(`http://nethush-garages/Click`, JSON.stringify({}));
  CloseGarage()
});
...
$(document).on('click', '.takeout', function(e) {
  ...
  if (CurrentMenu != null && CurrentMenu != undefined) {
    if (CurrentVehicleData != null && CurrentVehicleData != undefined) {
      CloseGarage()
      $.post(`http://nethush-garages/TakeOutVehicle`, JSON.stringify({Plate: CurrentVehicleData.Plate, Model: CurrentVehicleData.Model, State: CurrentVehicleData.State, Engine: CurrentVehicleData.Engine, Body: CurrentVehicleData.Body, Fuel: CurrentVehicleData.Fuel, Price: CurrentVehicleData.Price}));
    }
  }
});
...
$(document).on("click", '.vecicle-container', function(e) {
   ...
   if (CurrentMenu != null && CurrentMenu != undefined) {
     ...
     CurrentVehicleData = {Plate: $(this).data('plate'), Model: $(this).data('model'), State: $(this).data('stater'), Fuel: $(this).data('fuel'), Body: $(this).data('body'), Engine: $(this).data('motor'), Price: $(this).data('price')}
     CurrentMenu = $(this)
     ChoseActive = true
...
SetupGarageVehicles = function(data) {
  ...
  for (const [key, value] of Object.entries(data.garagevehicles)) {
    if (value['State'] === 'out') {
      var AddVehicle = '<div class="vecicle-container" data-plate='+value['Plate']+' data-model='+value['Model']+' data-stater='+value['State']+' data-fuel='+value['Fuel']+' data-motor='+value['Motor']+' data-body='+value['Body']+'><span class="vehicle-status status-out">Out</span><p><i class="fas fa-car"></i> '+ value['Name'] +'</p></div>'
      $('.menu-container').append(AddVehicle);
    } else {
      var AddVehicle = '<div class="vecicle-container" data-plate='+value['Plate']+' data-model='+value['Model']+' data-stater='+value['State']+' data-fuel='+value['Fuel']+' data-motor='+value['Motor']+' data-body='+value['Body']+'><span class="vehicle-status status-in">In</span><p><i class="fas fa-car"></i> '+ value['Name'] +'</p></div>'
      $('.menu-container').append(AddVehicle);
    }
  }
...
SetupDepotVehicles = function(data) {
  ...
  for (const [key, value] of Object.entries(data.depotvehicles)) {
      var AddVehicle = '<div class="vecicle-container" data-plate='+value['Plate']+' data-model='+value['Model']+' data-stater='+value['State']+' data-price='+value['Price']+' data-fuel='+value['Fuel']+' data-motor='+value['Motor']+' data-body='+value['Body']+'><span class="vehicle-status status-out"> €'+value['Price']+'</span><p><i class="fas fa-car"></i> '+ value['Name'] +'</p></div>'
      $('.menu-container').append(AddVehicle);
  }
  $(".main-container").css("display", "block");
  $(".main-container").animate({"right": "2vh"}, 550)
...
CloseGarage = function() {
  ...
  $.post(`http://nethush-garages/CloseNui`, JSON.stringify({}));
}
