    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if otherPlayer == offender then
            if tonumber(time) > 0 then
                ...
                TriggerEvent("swt_notifications:Infos","Time must be higher than 0..")
            end
        else
             TriggerEvent("swt_notifications:Infos","You're trying to send the wrong person to jail!..")
        end
    else
        TriggerEvent("swt_notifications:Infos","No one nearby!")
    end
end)
