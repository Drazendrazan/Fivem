...
RegisterServerEvent("mdt:ROpen")
AddEventHandler("mdt:ROpen", function()
	local src = source
	local Player = QBCore.Functions.GetPlayer(source)
	if Player.PlayerData.job.name == "police" then
    	exports['ghmattimysql']:execute("SELECT * FROM (SELECT * FROM `mdt_reports` ORDER BY `id` DESC LIMIT 3) sub ORDER BY `id` DESC", {}, function(reports)
...
AddEventHandler('mdt:server:BillPlayer', function(playerId, price)
    ...
    if Player.PlayerData.job.name == "police" then
        if OtherPlayer ~= nil then
            ...
            TriggerClientEvent('swt_notifications:Infos', OtherPlayer.PlayerData.source, "You received a fine of $"..price)
        end
    end
end)
