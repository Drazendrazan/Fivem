...
AddEventHandler('police:client:JailPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if tonumber(time) > 0 then
            ...
            TriggerEvent("swt_notifications:Infos","Time must be greater than 0..")
        end
    else
        TriggerEvent("swt_notifications:Infos","No one around!")
    end
    ...
    if not IsPedRagdoll(GetPlayerPed(-1)) then
        ...
        if player ~= -1 and distance < 1.5 then
            ...
            if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(GetPlayerPed(GetPlayerPed(-1))) then
                ...
                TriggerEvent("swt_notifications:Infos","You cannot cuff in a vehicle")
            end
        else
            TriggerEvent("swt_notifications:Infos","No one around!")
        end
        ...
        if player ~= -1 and distance < 1.5 then
            QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
                if result then 
                    ...
                    TriggerEvent("swt_notifications:Infos","You have no handcuffs with you")
                end
            end, "handcuffs")
        else
            TriggerEvent("swt_notifications:Infos","No one around!")
...
AddEventHandler('police:client:GetCuffed', function(playerId, isSoftcuff)
    if not isHandcuffed then
        ...
        if not isSoftcuff then
            ...
            TriggerEvent("swt_notifications:Infos","You are captivated!")
        else
            cuffType = 49
            GetCuffedAnimation(playerId)
            TriggerEvent("swt_notifications:Infos","You are now cuffed, but you can walk")
        end
    else
        isHandcuffed = false
        isEscorted = false
        ...
        ClearPedTasksImmediately(GetPlayerPed(-1))
        TriggerEvent("swt_notifications:Infos","You are uncuffed!")
    end
end)
