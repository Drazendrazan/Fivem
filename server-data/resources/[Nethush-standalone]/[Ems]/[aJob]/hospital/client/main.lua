                ...
                if IsControlJustReleased(0, Keys["E"]) then
                    if doctorCount > 1 then
                        ...
                        QBCore.Functions.Progressbar("hospital_checkin", "Checking in..", 2000, false, true, {
                            ...
                            --    TriggerEvent("swt_notifications:Infos","Bed is occupied..")
                                TriggerEvent("swt_notifications:Negative","SLMC System","Bed is occupied..","top-right",6000,true)

                            end
                        end, function() -- Cancel
                            TriggerEvent('animations:client:EmoteCommandStart', {"c"})
                        --    TriggerEvent("swt_notifications:Infos","Checking in failed!")
                            TriggerEvent("swt_notifications:Negative","SLMC System","Checking in failed!","top-right",6000,true)
            ...
            if closestBed ~= nil and not isInHospitalBed then
                if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations["beds"][closestBed].x, Config.Locations["beds"][closestBed].y, Config.Locations["beds"][closestBed].z, true) < 1.5) then
                    ...
                    if IsControlJustReleased(0, Keys["E"]) then
                        ...
                        --    TriggerEvent("swt_notifications:Infos","Beds occupied..")
                            TriggerEvent("swt_notifications:Negative","SLMC System","Bed is occupied..","top-right",6000,true)
...
AddEventHandler('hospital:client:Revive', function()
   ...
   -- TriggerEvent("swt_notifications:Infos","You are healed!")
    TriggerEvent("swt_notifications:Success","SLMC Sytem","You are healed!","top-right",6000,true)
...
AddEventHandler('hospital:client:HealInjuries', function(type)
    ...
    TriggerServerEvent("hospital:server:RestoreWeaponDamage")
  --  TriggerEvent("swt_notifications:Infos","Your wounds have been healed!")
    TriggerEvent("swt_notifications:Success","SLMC Sytem","Your wounds have been healed!","top-right",6000,true)
...
AddEventHandler('hospital:client:SendToBed', function(id, data, isRevive)
    ...
    Citizen.CreateThread(function ()
        ...
        local player = PlayerPedId()
        if isRevive then
           -- TriggerEvent("swt_notifications:Infos","You are getting healed..")
            TriggerEvent("swt_notifications:Success","SLMC Sytem","You are getting healed..","top-right",6000,true)
...
function DoLimbAlert()
    ...
    if not isDead then
        if #injured > 0 then
           ...
           -- TriggerEvent("swt_notifications:Infos","LimbDamageMsg, "primary", 5000)
            TriggerEvent("swt_notifications:Success","SLMC Sytem",limbDamageMsg,"top-right",6000,true)
...
function DoBleedAlert()
    ...
    if not isDead and tonumber(isBleeding) > 0 then
       -- TriggerEvent("swt_notifications:Infos","You are "..Config.BleedingStates[tonumber(isBleeding)].label)
        TriggerEvent("swt_notifications:Negative","SLMC System","You are "..Config.BleedingStates[tonumber(isBleeding)].label,"top-right",6000,true)
...
function OutfitsLijst()
    QBCore.Functions.TriggerCallback('apartments:GetOutfits', function(outfits)
        ...
        if outfits == nil then
           -- TriggerEvent("swt_notifications:Infos","You have no outfits saved...")
            TriggerEvent("swt_notifications:Negative","SLMC System","You have no outfits saved...","top-right",6000,true)
...
function selectOutfit(oData)
   ...
   -- TriggerEvent("swt_notifications:Infos",oData.outfitname.." outfit chosen")
    TriggerEvent("swt_notifications:Success","SLMC Sytem",oData.outfitname.." outfit chosen","top-right",6000,true)
...
function removeOutfit(oData)
   ...
   -- TriggerEvent("swt_notifications:Infos",oData.outfitname.." is deleted")
    TriggerEvent("swt_notifications:Success","SLMC Sytem",oData.outfitname.." is deleted","top-right",6000,true)
