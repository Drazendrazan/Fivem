...
function FormatAttachmentInfo(data) {
    $.post("http://nethush-inventory/GetWeaponData", JSON.stringify({
        weapon: data.name,
...
function handleAttachmentDrag() {
    ...
    $(".weapon-attachments-remove").droppable({
        ...
        drop: function(event, ui) {
            $.post('http://nethush-inventory/RemoveAttachment', JSON.stringify({
...
$(document).on('click', '#weapon-attachments', function(e){
    ...
    if (!Inventory.IsWeaponBlocked(ClickedItemData.name)) {
        ...
        $.post('http://nethush-inventory/Notify', JSON.stringify({
...
function handleDragDrop() {
    ...
    $(".item-slot").droppable({
        ...
        drop: function(event, ui) {
            ...
            $.post("http://nethush-inventory/UpdateStash", JSON.stringify({}));
        },
    });
    ...
    $("#item-use").droppable({
        ...
        drop: function(event, ui) {
            ...
            if(fromData.useable) {
                ...
                $.post("http://nethush-inventory/UseItem", JSON.stringify({
    ...
    $(".item-slot").click(function(event) {
        if (event.shiftKey) {
            ...
            $.post('http://nethush-inventory/UseItemShiftClick', JSON.stringify({
                slot: itemData.slot
            }));
        } 
    });
    ...
    $("#item-drop").droppable({
        ...
        drop: function(event, ui) {
            ...
            $.post("http://nethush-inventory/DropItem", JSON.stringify({
                inventory: fromInventory,
                item: fromData,
...
$(document).on('click', '.CombineItem', function(e){
    ...
    if (combineslotData.toData.combinable.anim != null) {
        $.post('http://nethush-inventory/combineWithAnim', JSON.stringify({
        ...
        $.post('http://nethush-inventory/combineItem', JSON.stringify({
...
function optionSwitch($fromSlot, $toSlot, $fromInv, $toInv, $toAmount, toData, fromData) {
    ...
    $.post("http://nethush-inventory/SetInventoryData", JSON.stringify({
...
function swap($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    if (fromData !== undefined && fromData.amount >= $toAmount) {       
        ...
        if((toData != undefined || toData != null) && toData.name == fromData.name && !fromData.unique) {
            ...
            $.post("http://nethush-inventory/PlayDropSound", JSON.stringify({}));
            $.post("http://nethush-inventory/SetInventoryData", JSON.stringify({
                fromInventory: $fromInv.attr("data-inventory"),
                toInventory: $toInv.attr("data-inventory"),
                fromSlot: $fromSlot,
            ...
            if (fromData.amount == $toAmount) {
                if (toData != undefined && toData.combinable != null && isItemAllowed(fromData.name, toData.combinable.accept)) {
                    $.post('http://nethush-inventory/getCombineItem', JSON.stringify({item: toData.combinable.reward}), function(item){
...
function InventoryError($elinv, $elslot) {
    ...
    $.post("http://nethush-inventory/PlayDropFail", JSON.stringify({}));
}
...
(() => {
    ...
    Inventory.Close = function() {
        ...
        $.post("http://nethush-inventory/CloseInventory", JSON.stringify({}));
...
$(document).on('click', '#rob-money', function(e){
    ...
    $.post('http://nethush-inventory/RobMoney', JSON.stringify({
        TargetId: TargetId
    }));
    $("#rob-money").remove();
});