...
--[[RegisterNetEvent('nethush-items:client:use:joint')
AddEventHandler('nethush-items:client:use:joint', function()
  Citizen.SetTimeout(1000, function()
    QBCore.Functions.Progressbar("smoke-joint", "Smoking joint..", 4500, false, true, {
     ...
     disableCombat = true,
        ...
        TriggerEvent("nethush-inventory:client:ItemBox", QBCore.Shared.Items["joint"], "remove")
        TriggerEvent('nethush-items:client:joint:effect')
...
RegisterNetEvent("nethush-items:client:use:oxy")
AddEventHandler("nethush-items:client:use:oxy", function()
  Citizen.SetTimeout(1000, function()
    QBCore.Functions.Progressbar("snort_coke", "Taking Oxy..", 1500, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
       ...
       flags = 49,
        ...
        TriggerEvent("nethush-inventory:client:ItemBox", QBCore.Shared.Items["oxy"], "remove")
...
RegisterNetEvent("nethush-items:client:use:coke")
AddEventHandler("nethush-items:client:use:coke", function()
  Citizen.SetTimeout(1000, function()
    QBCore.Functions.Progressbar("snort_coke", "Snorting coke..", math.random(5000, 8000), false, true, {
        ...
        TriggerServerEvent("QBCore:Server:RemoveItem", "coke-bag", 1)
        TriggerEvent("nethush-inventory:client:ItemBox", QBCore.Shared.Items["coke-bag"], "remove")
        TriggerEvent("qb-police:client:SetStatus", "widepupils", 200)
        CokeBagEffect()
    end, function() -- Cancel
        StopAnimTask(PlayerPedId(), "switch@trevor@trev_smoking_meth", "trev_smoking_meth_loop", 1.0)
...
RegisterNetEvent("nethush-items:client:use:lsd")
AddEventHandler("nethush-items:client:use:lsd", function()
  Citizen.SetTimeout(1000, function()
    QBCore.Functions.Progressbar("snort_coke", "Using LSD..", 3000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
       ...
       flags = 49,
        ...
        StopAnimTask(PlayerPedId(), "mp_suicide", "pill", 1.0)
        TriggerServerEvent("QBCore:Server:RemoveItem", "lsd-strip", 1)
        TriggerEvent("nethush-inventory:client:ItemBox", QBCore.Shared.Items["lsd-strip"], "remove")
        TriggerEvent("qb-police:client:SetStatus", "widepupils", 200)
        Effectlsd()
    end, function() -- Cancel
        StopAnimTask(PlayerPedId(), "mp_suicide", "pill", 1.0)
...
RegisterNetEvent('nethush-items:client:joint:effect')
AddEventHandler('nethush-items:client:joint:effect', function()
  OnWeed = true
  Time = 15
  while OnWeed do
    if Time > 0 then
     Citizen.Wait(1000)
     Time = Time - 1
     TriggerServerEvent('qb-hud:Server:RelieveStress', math.random(2, 6))
    end
     if Time <= 0 then
      OnWeed = false
     end 
