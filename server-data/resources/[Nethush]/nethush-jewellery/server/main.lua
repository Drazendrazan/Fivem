...
AddEventHandler('qb-jewellery:server:vitrineReward', function()
    ...
    if otherchance == odd then
        ...
        if Player.Functions.AddItem(Config.VitrineRewards[item]["item"], amount) then
            ...
            TriggerClientEvent('swt_notifications:Infos', src, 'You\'re ..')
        end
    else
        local amount = math.random(2, 4)
        if Player.Functions.AddItem("10kgoldchain", amount) then
            TriggerClientEvent('nethush-inventory:client:ItemBox', src, QBCore.Shared.Items["10kgoldchain"], 'add')
        else
            TriggerClientEvent('swt_notifications:Infos', src, 'You are carrying to much..')
        end
    end
end)
