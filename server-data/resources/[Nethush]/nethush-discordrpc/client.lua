	...
	while true do
		...
		if config.mainTextType == 1 then
			...
			if playerCount > 1 then
				SetRichPresence("" .. playerCount .. " Citizens")
			else
				SetRichPresence("1 Citizen")
			end	
		...
		SetDiscordRichPresenceAsset('giphy')
		...
		if clientHide == true then 
			...
			elseif IsPedOnFoot(PlayerPedId()) == false then
				...
				elseif (IsPedInAnyVehicle(GetPlayerPed(-1)) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1), false), -1) == GetPlayerPed(-1) and (vehicles:find(GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsIn(GetPlayerPed(-1), false)))) or vehicles == "")) then
					SetDiscordRichPresenceAssetSmall('tesla')
					if vehspeed <= 2 then
						SetDiscordRichPresenceAssetSmallText("Parked in a Tesla at " .. streetName .. "")
					else
						SetDiscordRichPresenceAssetSmallText("Driving a Tesla at " .. vehspeed .. "mph down " .. streetName .. "")
					end
				...
				elseif vehicleClass == 18 then
					SetDiscordRichPresenceAssetSmall('emergencyvehicle')
