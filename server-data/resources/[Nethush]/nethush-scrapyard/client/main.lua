							...
							if IsControlJustReleased(0, Keys["E"]) then
								if GetPedInVehicleSeat(vehicle, -1) == GetPlayerPed(-1) then
									if IsVehicleValid(GetEntityModel(vehicle)) then 
										...
										TriggerEvent("swt_notifications:Infos","This vehicle can\'t be scrapped..")
									end
								else
									TriggerEvent("swt_notifications:Infos","You're not the driver..")
								end
							end
						end
					end
...
function CreateListEmail()
	if Config.CurrentVehicles ~= nil and next(Config.CurrentVehicles) ~= nil then 
		...
		TriggerEvent("swt_notifications:Infos","You are not allowed to Destroy vehicles now..")
	end
end
