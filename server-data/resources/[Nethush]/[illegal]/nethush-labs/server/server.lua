...
QBCore.Functions.CreateCallback('nethush-illegal:server:get:config', function(source, cb)
    cb(Config)
end)
...
QBCore.Functions.CreateCallback('nethush-illegal:serverhas:robbery:item', function(source, cb)
...
QBCore.Functions.CreateCallback('nethush-illegal:server:has:drugs', function(source, cb)
    local src = source
...
QBCore.Functions.CreateCallback('nethush-illegal:server:get:drugs:items', function(source, cb)
    local src = source
...
RegisterServerEvent('nethush-illegal:server:unpack:coke')
AddEventHandler('nethush-illegal:server:unpack:coke', function()
    local Player = QBCore.Functions.GetPlayer(source)
    if Player.Functions.RemoveItem('packed-coke-brick', 1) then
        Player.Functions.AddItem('cokebaggy', math.random(10, 20))
        TriggerClientEvent('nethush-inventory:client:ItemBox', source, QBCore.Shared.Items['pure-coke-brick'], "add")
        TriggerClientEvent('nethush-inventory:client:ItemBox', source, QBCore.Shared.Items['packed-coke-brick'], "remove")
    end
end)

RegisterServerEvent('nethush-illegal:server:finish:corner:selling')
AddEventHandler('nethush-illegal:server:finish:corner:selling', function(Price, ItemName, ItemAmount)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player.Functions.RemoveItem(ItemName, ItemAmount) then
        Player.Functions.AddMoney('cash', Price, 'corner-selling')
        TriggerClientEvent('nethush-inventory:client:ItemBox', source, QBCore.Shared.Items[ItemName], "remove")
    end
end)
                ...
                SetTimeout(SellData['Wait'], function()
                    if Config.Labs[LabId]['Inventory'][Slot] ~= nil then
                        ...
                        if SellData['Success'] == 'coke-powder' then
                            TriggerClientEvent('nethush-illegal:client:play:sound:coke', -1)
                        end
                        TriggerClientEvent('nethush-illegal:client:sync:inventory', -1, LabId, Config.Labs[LabId]['Inventory'])
                    end
                end)
                if Amount > 1 then
...
RegisterServerEvent('nethush-illegal:server:add:ingredient')
AddEventHandler('nethush-illegal:server:add:ingredient', function(LabId, IngredientName, Bool, Amount)
  ...
  if Config.Labs[LabId]['Ingredients']['meth-ingredient-1'] and Config.Labs[LabId]['Ingredients']['meth-ingredient-2'] then
    ...
    TriggerClientEvent('nethush-illegal:client:start:cooking', -1, LabId)
  end
  TriggerClientEvent('nethush-illegal:client:sync:meth', -1, Config.Labs[LabId], LabId, false)
end)

RegisterServerEvent('nethush-illegal:server:get:meth')
AddEventHandler('nethush-illegal:server:get:meth', function(RandomAmount, LabId)
 local src = source
 local Player = QBCore.Functions.GetPlayer(src)
 ResetMethLab(LabId)
 Citizen.SetTimeout(150, function()
    Player.Functions.AddItem('meth-powder', RandomAmount)
    TriggerClientEvent('nethush-inventory:client:ItemBox', src, QBCore.Shared.Items['meth-powder'], "add")
 end)
end)

RegisterServerEvent('nethush-illegal:server:reset:meth')
AddEventHandler('nethush-illegal:server:reset:meth', function(LabId)
    ResetMethLab(LabId)
end)
...
function ResetMethLab(LabId)
 ...
 TriggerClientEvent('nethush-illegal:client:sync:meth', -1, Config.Labs[LabId], LabId, true)
end
...
RegisterServerEvent('nethush-illegal:server:add:printer:item')
AddEventHandler('nethush-illegal:server:add:printer:item', function(LabId, ItemType, Amount)
    Config.Labs[LabId][ItemType] = Config.Labs[LabId][ItemType] + Amount
    TriggerClientEvent('nethush-illegal:client:sync:items', -1, ItemType, Config.Labs[LabId][ItemType])
end)

RegisterServerEvent('nethush-illegal:server:remove:printer:item')
AddEventHandler('nethush-illegal:server:remove:printer:item', function(LabId, ItemType, Amount)
    Config.Labs[LabId][ItemType] = Config.Labs[LabId][ItemType] - Amount
    TriggerClientEvent('nethush-illegal:client:sync:items', -1, ItemType, Config.Labs[LabId][ItemType])
end)

RegisterServerEvent('nethush-illegal:server:set:printer:money')
AddEventHandler('nethush-illegal:server:set:printer:money', function(LabId, Amount)
    Config.Labs[LabId]['Total-Money'] = Config.Labs[LabId]['Total-Money'] + Amount
    TriggerClientEvent('nethush-illegal:client:sync:money', -1, Config.Labs[LabId]['Total-Money'])
end)

RegisterServerEvent('nethush-illegal:server:get:money:printer:money')
AddEventHandler('nethush-illegal:server:get:money:printer:money', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    Player.Functions.AddMoney('cash', Config.Labs[3]['Total-Money'], 'money-printer')
    Config.Labs[3]['Total-Money'] = 0
    TriggerClientEvent('nethush-illegal:client:sync:money', -1, Config.Labs[3]['Total-Money'])
end)

--- Electrnics 

RegisterServerEvent('nethush-illegal:server:sell:electrnoics')
AddEventHandler('nethush-illegal:server:sell:electrnoics', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    for k, v in pairs(Player.PlayerData.items) do
        if v.name == 'stolen-tv' then
            ...
            TriggerClientEvent('nethush-inventory:client:ItemBox', Player.PlayerData.source, QBCore.Shared.Items['stolen-tv'], "remove")
        elseif v.name == 'stolen-micro' then
            Player.Functions.RemoveItem('stolen-micro', 1)
            Player.Functions.AddItem('money-roll', math.random(50, 85))
            Player.Functions.AddMoney('cash', math.random(1500, 2100), 'sold-stolen-goods')
            TriggerClientEvent('nethush-inventory:client:ItemBox', Player.PlayerData.source, QBCore.Shared.Items['stolen-micro'], "remove")
        elseif v.name == 'stolen-pc' then
            Player.Functions.RemoveItem('stolen-pc', 1)
            Player.Functions.AddItem('money-roll', math.random(50, 85))
            Player.Functions.AddMoney('cash', math.random(1500, 2100), 'sold-stolen-goods')
            TriggerClientEvent('nethush-inventory:client:ItemBox', Player.PlayerData.source, QBCore.Shared.Items['stolen-pc'], "remove")
        end
    end
end)