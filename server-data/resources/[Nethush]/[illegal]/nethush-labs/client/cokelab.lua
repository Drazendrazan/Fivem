                ...
                if Distance < 2.0 then
                    ...
                    if IsControlJustReleased(0, 38) then
                        local CokeLab = {}
                         ...
                         CokeLab.slots = 2
                        TriggerServerEvent("nethush-inventory:server:OpenInventory", "lab", CurrentLab, CokeLab)
                    end
                end
            ...
            if InsideLab and Config.Labs[CurrentLab]['Name'] == 'Cokelab' then
                ...
                if Distance < 1.2 then
                    ...
                    if IsControlJustReleased(0, 38) then
		    			...
		    			TriggerServerEvent("nethush-inventory:server:OpenInventory", "cokecrafting", math.random(1, 99), CokeCrafting)
                    end
                end
...
RegisterNetEvent('nethush-illegal:client:start:burner-call')
AddEventHandler('nethush-illegal:client:start:burner-call', function()
    if not DoingSomething then
        ...
        Citizen.SetTimeout(1250, function()
            ...
            exports['nethush-assets']:RequestAnimationDict("cellphone@")
            TaskPlayAnim(PlayerPedId(), 'cellphone@', 'cellphone_text_to_call', 3.0, 3.0, -1, 50, 0, false, false, false)
            local Time = 5
            repeat
              Time = Time -1
              TriggerEvent("nethush-sound:client:play", "death", 0.2)
              Citizen.Wait(3500)
            until Time == 0
            if RandomValue == 1 then
                TriggerEvent("nethush-sound:client:play", "call-1", 0.5) 
            ...
            Citizen.SetTimeout(14000, function()
                ...
                TriggerServerEvent('QBCore:Server:RemoveItem', 'burner-phone', 1)
                TriggerEvent("nethush-inventory:client:ItemBox", QBCore.Shared.Items['burner-phone'], "remove")
                TriggerServerEvent('qb-phone:server:sendNewMail', {
                    ...
                    button = {
                        enabled = true,
                        buttonEvent = "nethush-illegal:client:start:pickup:coke",
...
RegisterNetEvent('nethush-illegal:client:start:pickup:coke')
AddEventHandler('nethush-illegal:client:start:pickup:coke', function(PickupData)
    local DoingJob = true
    ...
    while DoingJob do
        ...
        if Distance <= 2.0 then
            ...
            if IsControlJustReleased(0, 38) then
                ...
                TriggerServerEvent("nethush-inventory:server:OpenInventory", "shop", "Cokebrick", ItemShop)
...
RegisterNetEvent('nethush-illegal:client:play:sound:coke')
AddEventHandler('nethush-illegal:client:play:sound:coke', function()
    if InsideLab and NearAction then
        TriggerEvent('nethush-sound:client:play', 'cutting', 0.4)
    end
end)
