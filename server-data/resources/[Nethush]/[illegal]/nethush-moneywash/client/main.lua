...
Citizen.CreateThread(function()
 ...
 --               Text3D(1135.92, -989.49, 46.11, '<FONT FACE="Fira Sans">~g~[G]~w~ Wash Money')
				...
				TriggerServerEvent("nethush-moneywash:server:checkmoney")
                end
            end
...
RegisterNetEvent('nethush-moneywash:client:WashProggress')
AddEventHandler('nethush-moneywash:client:WashProggress', function(source)
    ...
    TriggerServerEvent("nethush-moneywash:server:getmoney")
    end, function() -- Cancel
    StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_ped", 1.0)
