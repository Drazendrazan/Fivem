                            ...
                            if IsPedInAnyVehicle(PlayerPedId(), false) then
                                if GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId()), -1) == PlayerPedId() then
                                    if isTruckerVehicle(GetVehiclePedIsIn(PlayerPedId(), false)) then
                                        ...
                                        TriggerEvent('swt_notifications:Infos','This is not what you are supposed to be doing!')
                                    end
                                else
                                    TriggerEvent('swt_notifications:Infos','You need to be the driver..')
                                end
                            else
                                MenuGarage()
                    ...
                    if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z, true) < 1.5) then
                        ...
                        if IsControlJustReleased(0, Keys["E"]) then
                            if JobsDone > 0 then
                                ...
                                TriggerEvent("swt_notifications:Infos","You didnt do any work..")
                            end
                        end
                            ...
                            if not hasBox then
                                ...
                                if isTruckerVehicle(vehicle) and CurrentPlate == GetVehicleNumberPlateText(vehicle) then
                                    ...
                                    if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, trunkpos.x, trunkpos.y, trunkpos.z, true) < 1.5 and not isWorking then
                                        ...
                                        if IsControlJustReleased(0, Keys["E"]) then
                                            isWorking = true
                                            QBCore.Functions.Progressbar("work_carrybox", "Grabbing products..", 2000, false, true, {
                            ...
                            elseif hasBox then
                                if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, CurrentLocation.x, CurrentLocation.y, CurrentLocation.z, true) < 1.5 and not isWorking then
                                    ...
                                    if IsControlJustReleased(0, Keys["E"]) then
                                        ...
                                        QBCore.Functions.Progressbar("work_dropbox", "Handing over products..", 2000, false, true, {
                                            disableMovement = true,
                                            disableCarMovement = true,
                                            disableMouse = false,
                                            disableCombat = true,
                                            ...
                                            if currentCount == CurrentLocation.dropcount then
                                                ...
                                                TriggerEvent("swt_notifications:Infos","You have delivered all products, go to the next location.")
                                                if CurrentBlip ~= nil then
                                                    RemoveBlip(CurrentBlip)
                                                    CurrentBlip = nil
                                                end
...
function getNewLocation()
    ...
    if location ~= 0 then
        ...
        TriggerEvent("swt_notifications:Infos","You have delivered all the products from your truck.. Time for your pay check! go back to the hub")
        if CurrentBlip ~= nil then
            RemoveBlip(CurrentBlip)
            CurrentBlip = nil
        end
