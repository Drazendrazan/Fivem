...
AddEventHandler("gangs:init", function()
    ...
    if xPlayer then
        ...
        local results = QBCore.Functions.ExecuteSql(true, "SELECT * FROM `gangs_members` WHERE `cid` = '" .. CID .. "'")
        local gang = { name = "none", grade = "none", char = char }
        
        if results[1] then
            gang =  { name = results[1].gang, grade = results[1].grade, char = char, id = CID }
            Members[CID] = gang
            QBCore.Functions.ExecuteSql(true, "UPDATE `gangs_members` SET `char` = '" .. char .. "' WHERE `cid`='" .. CID .. "'")
        end
...
AddEventHandler("gangs:updateGrade", function(member, grade)
    ...
    local char = nil
    local xTarget = QBCore.Functions.GetPlayerByCitizenId(member)
    if xTarget then
        char = xTarget.PlayerData.charinfo.firstname .. " " .. xTarget.PlayerData.charinfo.lastname
    else
        local results = QBCore.Functions.ExecuteSql(true, "SELECT * FROM `gangs_members` WHERE `cid` = '" .. member .. "'")
        if results[1] then
            char = results[1].char
        end
    end
    ...
    if char then
        ...
        if grade ~= "none" then
            if QBCore.Functions.ExecuteSql(true, "UPDATE `gangs_members` SET `gang` = '" .. xGang.name .. "', `grade` = '" .. grade .. "', `char` = '" .. char .. "' WHERE `cid`='" .. member .. "'")['affectedRows'] <= 0 then
                QBCore.Functions.ExecuteSql(false, "INSERT INTO `gangs_members` (`cid`, `gang`, `grade`, `char`) VALUES ('" .. member .. "','" .. xGang.name .. "', '" .. grade .. "', '" .. char .. "')")
            end
    
            Members[member] = gang
        else
            Members[member] = nil
            QBCore.Functions.ExecuteSql(false, "DELETE FROM `gangs_members` WHERE `cid`='" .. member .. "'")
        end
...
AddEventHandler("gangs:saveMoney", function()
    ...
    if money then
        if QBCore.Functions.ExecuteSql(true, "UPDATE `gangs_money` SET `amount` = '" .. money .. "' WHERE `gang`='" .. xGang.name .. "'")['affectedRows'] <= 0 then
            QBCore.Functions.ExecuteSql(false, "INSERT INTO `gangs_money` (`gang`, `amount`) VALUES ('" .. xGang.name .. "', '" .. money .. "')")
        end
    end
end)
...
QBCore.Commands.Add("gang", "Check your gang.", {}, true, function(source, args)
    local src = source
    local CID = CIDS[src]
...
QBCore.Commands.Add("setgang", "Give a gang to a player.", {{name="id", help="Player ID"}, {name="gang", help="Gang name"}, {name="grade", help="Grade name"}}, true, function(source, args)
    local src = source
    ...
    if Player ~= nil then
        ...
        --if Gang ~= nil and Grade ~= nil then
            ...
            if args[2] ~= "none" then
                if QBCore.Functions.ExecuteSql(true, "UPDATE `gangs_members` SET `gang` = '" .. args[2] .. "', `grade` = '" .. args[3] .. "', `char` = '" .. char .. "' WHERE `cid`='" .. CID .. "'")['affectedRows'] <= 0 then
                    QBCore.Functions.ExecuteSql(false, "INSERT INTO `gangs_members` (`cid`, `gang`, `grade`, `char`) VALUES ('" .. CID .. "','" .. args[2] .. "', '" .. args[3] .. "', '" .. char .. "')")
                end
            else
                Members[CID] = nil
                QBCore.Functions.ExecuteSql(false, "DELETE FROM `gangs_members` WHERE `cid`='" .. CID .. "'")
            end
        --else
...
CreateThread(function()
    Wait(500)
    local results = QBCore.Functions.ExecuteSql(true, "SELECT * FROM `gangs_members`")
    for i=1, #results do
        local result = results[i]
        Members[result.cid] = { name = result.gang, grade = result.grade, char = result.char, id = result.cid }
    end

    local accounts = QBCore.Functions.ExecuteSql(true, "SELECT * FROM `gangs_money`")
    for i=1, #accounts do
