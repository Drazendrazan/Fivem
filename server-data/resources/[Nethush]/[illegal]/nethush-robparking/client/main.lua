...
function lockpickDone(success)
    ...
    if success then
        ...
        if math.random(1, 100) <= 40 and IsWearingHandshoes() then
            TriggerServerEvent("evidence:server:CreateFingerDrop", pos)
           -- TriggerEvent("swt_notifications:Infos","You ripped your gloves..")
            TriggerEvent("swt_notifications:Warning","SLMC System","You ripped your gloves..","top-right",3000,true)
        end
    ...
    while true do
        ...
        if IsNearParkingMeter() then
            ...
            if IsControlJustPressed(1, Keys["E"])  then
                if (IsInVehicle()) then
                   -- TriggerEvent('swt_notifications:Infos','You cannot rob a parking meter in a vehicle!')
                    TriggerEvent("swt_notifications:Warning","SLMC System",'You cannot rob a parking meter in a vehicle!',"top-right",3000,true)
                else
                    QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
                        ...
                        if result == true then
                            if canRob == true then
                               ...
                               -- TriggerEvent('swt_notifications:Infos','You are robbing a parking meter too soon, you have to wait '..timer..' seconds')
                                TriggerEvent("swt_notifications:Warning","SLMC System",'You are robbing a parking meter too soon, you have to wait '..timer..' seconds',"right",3000,true)
                                TriggerServerEvent("InteractSound_SV:PlayOnSource", "notif", 0.25)
                            end
                        else
                           -- TriggerEvent('swt_notifications:Infos','You do not have the necessary tools!')
                               -- TriggerEvent("swt_notifications:Warning","SLMC System","You do not have the necessary tools!","top-right",3000,true)
                                    TriggerEvent("swt_notifications:Negative","SLMC System","You do not have the necessary tools!","right",3000,true)
                                        TriggerServerEvent("InteractSound_SV:PlayOnSource", "notif", 0.25)
