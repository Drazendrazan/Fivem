...
AddEventHandler(
    ...
    function()
        ...
        QBCore.Functions.TriggerCallback(
            "nethush-pacific:server:GetConfig",
            function(config)
                Config = config
            end
        )
...
Citizen.CreateThread(
    function()
        ...
        local requiredItems = {
            [1] = {
                name = QBCore.Shared.Items["electronickit"]["name"],
                image = QBCore.Shared.Items["electronickit"]["image"]
            },
            [2] = {
                name = QBCore.Shared.Items["trojan_usb"]["name"],
                image = QBCore.Shared.Items["trojan_usb"]["image"]
            }
        }
        local Alarm = {
            [1] = {
                name = QBCore.Shared.Items["black-card"]["name"],
                image = QBCore.Shared.Items["black-card"]["image"]
            }
        }
        local ExplosiveItem = {
            [1] = {
                name = QBCore.Shared.Items["explosive"]["name"],
                image = QBCore.Shared.Items["explosive"]["image"]
            }
        }
        while true do
                ...
                if Config.PacificB["lights"] then
                    ...
                    if Area < 2.0 then
                        ...
                        if not AlarmItem then
                            AlarmItem = true
                            TriggerEvent("nethush-inventory:client:requiredItems", Alarm, true)
                        end
                    else
                        if AlarmItem then
                            AlarmItem = false
                            TriggerEvent("nethush-inventory:client:requiredItems", Alarm, false)
                        end
                    end
                else
...
RegisterNetEvent("nethush-pacific:client:PoliceAlertMessage")
AddEventHandler(
    "nethush-pacific:client:PoliceAlertMessage",
    function(title, coords, blip)
        if blip then
            TriggerEvent(
                "nethush-policealerts:client:AddPoliceAlert",
                {
                    timeOut = 5000,
                    alertTitle = title,
                    details = {
...
RegisterNetEvent("nethush-pacific:client:Klapdebank")
AddEventHandler(
    "nethush-pacific:client:Klapdebank",
    function(state)
...
RegisterNetEvent("nethush-pacific:client:setfirstState")
AddEventHandler(
    "nethush-pacific:client:setfirstState",
    function(state)
        Config.PacificB["explosive"]["isOpened"] = state
        TriggerServerEvent("nethush-pacific:server:setTimeout")
    end
)

RegisterNetEvent("nethush-pacific:client:setLockerState")
AddEventHandler(
    "nethush-pacific:client:setLockerState",
    function(lockerId, state, bool)
        Config.PacificB["lockers"][lockerId][state] = bool
    end
)

RegisterNetEvent("nethush-pacific:client:set:lights:state")
AddEventHandler(
    "nethush-pacific:client:set:lights:state",
    function(bool)
        Config.PacificB["lights"] = bool
    end
)

RegisterNetEvent("nethush-pacific:client:set:trol:state")
AddEventHandler(
    "nethush-pacific:client:set:trol:state",
    function(state)
...
RegisterNetEvent("nethush-pacific:client:DoSmokePfx")
AddEventHandler(
    "nethush-pacific:client:DoSmokePfx",
    function()
...
RegisterNetEvent("nethush-pacific:client:clear:trollys")
AddEventHandler(
    "nethush-pacific:client:clear:trollys",
    function()
        Count = 0
...
RegisterNetEvent("nethush-pacific:client:set:trolly:state")
AddEventHandler(
    "nethush-pacific:client:set:trolly:state",
