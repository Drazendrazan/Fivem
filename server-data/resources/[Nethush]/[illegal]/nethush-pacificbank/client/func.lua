function LockDownStart()
    ...
    if not copsCalled then
        if Config.PacificB["alarm"] then
            --TriggerServerEvent("nethush-pacific:server:callCops", GetEntityCoords(PlayerPedId()))
            copsCalled = true
        end
    end
    -- SpawnSecurity()
    StartLockDownEscape()
    TriggerServerEvent("nethush-doorlock:server:updateState", 35, true)
    TriggerServerEvent("nethush-doorlock:server:updateState", 36, true)
end
...
function GetMoneyFromTrolly(TrollyNumber)
    ...
    GrabbingMoney = false
    TriggerServerEvent("nethush-pacific:server:set:trolly:state", TrollyNumber, true)
    TriggerServerEvent("nethush-pacific:server:rob:pacific:money")
