	...
	RegisterCommand(Config.AdvertisementCommand, function(source, args, rawCommand)
		...
		if canAdvertise then
			if bankMoney >= Config.AdvertisementPrice then
				...
				TriggerClientEvent('nethush-notify:Alert', source, "ADVERTISEMENT", "Advertisement successfully made for "..Config.AdvertisementPrice..'€', 10000, 'success')
				...
				TriggerClientEvent('nethush-notify:Alert', source, "ADVERTISEMENT", "You don't have enough money to make an advertisement", 10000)
			end
		else
			TriggerClientEvent('nethush-notify:Alert', source, "ADVERTISEMENT", "You can't advertise so quickly", 10000)
		end
	end)
end
