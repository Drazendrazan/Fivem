window.APP = {
	...
	mounted() {
		post('https://nethush-Chat/loaded', JSON.stringify({}));
	...
	methods: {
		...
		send(e) {
			if(this.message !== '') {
				post('https://nethush-Chat/chatResult', JSON.stringify({
		...
		hideInput(canceled = false) {
			if (canceled) {
				post('https://nethush-Chat/chatResult', JSON.stringify({ canceled }));
			}
			this.message = '';
