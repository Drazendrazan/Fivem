...
AddEventHandler('HIJINX-Pilotjob:server:DoBail', function(bool, vehInfo)
    ...
    if bool then
        if Player.PlayerData.money.cash >= Config.BailPrice then
            ...
            TriggerClientEvent('swt_notifications:Infos', src, 'You paid your deposit of $1000 (cash)')
        ...
        elseif Player.PlayerData.money.bank >= Config.BailPrice then
            ...
            TriggerClientEvent('swt_notifications:Infos', src, 'You paid your deposit of $1000 (bank)')
            TriggerClientEvent('HIJINX-Pilotjob:client:SpawnVehicle', src, vehInfo)
        else
            TriggerClientEvent('swt_notifications:Infos', src, 'You dont have enough cash, the deposit is $1000.')
        end
    else
        if Bail[Player.PlayerData.citizenid] ~= nil then
            Player.Functions.AddMoney('cash', Bail[Player.PlayerData.citizenid], "pilotjob-bail-paid")
            Bail[Player.PlayerData.citizenid] = nil
            TriggerClientEvent('swt_notifications:Infos', src, 'You received your deposit back.')
        end
    end
end)
