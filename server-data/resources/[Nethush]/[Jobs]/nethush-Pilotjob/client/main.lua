                            ...
                            if IsPedInAnyVehicle(PlayerPedId(), false) then
                                if GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId()), -1) == PlayerPedId() then
                                    if isTruckerVehicle(GetVehiclePedIsIn(PlayerPedId(), false)) then
                                        ...
                                        TriggerEvent('swt_notifications:Infos','This is not the plane!')
                                    end
                                else
                                    TriggerEvent('swt_notifications:Infos','You must be a driver..')
                                end
                            else
                                MenuGarage()
                                    ...
                                    if IsControlJustReleased(0, Keys["E"]) then
                                        ...
                                        QBCore.Functions.Progressbar("work_dropbox", "Dropping Off Cargo..", 2000, false, true, {
                                            ...
                                            if currentCount == CurrentLocation.dropcount then
                                                ...
                                                TriggerEvent("swt_notifications:Infos","You have delivered all cargo, on to the next point")
                                                if CurrentBlip ~= nil then
                                                    RemoveBlip(CurrentBlip)
                                                    CurrentBlip = nil
                                                end
...
function getNewLocation()
    ...
    if location ~= 0 then
        ...
        TriggerEvent("swt_notifications:Infos","You went to all the runways. Time for your payslip!")
        if CurrentBlip ~= nil then
            RemoveBlip(CurrentBlip)
            CurrentBlip = nil
        end
