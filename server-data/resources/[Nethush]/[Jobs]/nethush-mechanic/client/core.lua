...
CreateThread(function()
    for i = 1, #Config.Positions, 1 do
        ...
        if (tempPos.hasBlip == nil or tempPos.hasBlip == true) then
           -- addBlip(tempPos.pos, 72, 5, 'Mechanic2')
        end
    end
...
function updateCash()
    ...
    CurrentVehicleData = GetVehicleData(playerVeh)
...
function openUI()
    ...
    if (playerVeh ~= 0 and DoesEntityExist(playerVeh)) then
       ...
       -- customVehicleData = QBCore.Functions.GetVehicleProperties(customVehicle)
...
function closeUI(sendToUI, resetVehToDefault)
    ...
    if (resetVehToDefault == 1) then
        ...
        QBCore.Functions.SetVehicleProperties(customVehicle, customVehicleData)
    end
        ...
        if (data.what == 'menu') then
            ...
            elseif (data.user == 'enter') then
                ...
                if ((not isOpenByAdmin) and menuOption and menuOption.price) then
                    ...
                    elseif (menuOption.price > 0) then
                        ...
                        if (QBCore and (QBCore.Functions.GetPlayerData()['money']['cash'] >= tempPrice)) then
                            ...
                            TriggerEvent("swt_notifications:Negative","SLMC System","You Don't Have Money on Pocket...","right",3000,true)
                            canBuyMod = false
                        end
                    end
                end
                ...
                if (data.color ~= nil) then
                 ...
                 --   QBCore.Functions.TriggerCallback('qb-customs:server:SaveVehicleProps', function(result)
                  --  end, CurrentVehicleData)
                  TriggerServerEvent('qb-customs:server:SaveVehicleProps', customVehicleData)
    ...
    for i = startIndex, GetNumVehicleModData(customVehicle, menuOption.modType), 1 do
        ...
        if (curOption == i) then
            ...
            if (type(menuOption.priceMult) == 'number') then
                tempPrice = math.floor(customVehiclePrice)
            else
                tempPrice = math.floor(customVehiclePrice / 10)
            end
        end
