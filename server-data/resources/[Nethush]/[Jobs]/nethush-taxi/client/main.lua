    ...
    if whitelistedVehicle() then
        if NpcData.CountDown == 180 then
            if not NpcData.Active then
                ...
                TriggerEvent('swt_notifications:Infos','The NPC is indicated on your navigation!')
                        ...
                        if dist < 20 then
                            ...
                            if dist < 5 then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    ...
                                    TriggerEvent('swt_notifications:Infos','Bring the NPC to the specified location.')
                                    if NpcData.NpcBlip ~= nil then
                                        RemoveBlip(NpcData.NpcBlip)
                                    end
                                    GetDeliveryLocation()
                ...
                TriggerEvent('swt_notifications:Infos','You are already on an NPC mission..')
            end
        else
            TriggerEvent('swt_notifications:Infos','There are no NPC\'s available..')
        end
    else
        TriggerEvent('swt_notifications:Infos','Youre not in a Taxi :(')
    end
end)
            ...
            if dist < 20 then
                ...
                if dist < 5 then
                    ...
                    if IsControlJustPressed(0, Keys["E"]) then
                        ...
                        TriggerEvent('swt_notifications:Infos','Person delivered in top condition')
                        if NpcData.DeliveryBlip ~= nil then
                            RemoveBlip(NpcData.DeliveryBlip)
                        end
...
AddEventHandler('qb-taxi:client:toggleMeter', function()
    ...
    if IsPedInAnyVehicle(ped, false) then
        if whitelistedVehicle() then
            ...
            TriggerEvent('swt_notifications:Infos','This vehicle does not have a Taxi Meter..')
        end
    else
...
AddEventHandler('qb-taxi:client:enableMeter', function()
    ...
    if meterIsOpen then
        ...
        TriggerEvent('swt_notifications:Infos','The Taxi Meter is not active..')
    end
end)
...
AddEventHandler('qb-taxi:client:toggleMuis', function()
    ...
    if meterIsOpen then
        ...
        TriggerEvent('swt_notifications:Infos','No Taxi Meter in sight..')
    end
end)
