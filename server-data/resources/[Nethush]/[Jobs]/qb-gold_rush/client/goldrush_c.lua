...
local model1 = 'teslapd' --model
local delXAU = -666.55  --del auto 
local delYAU = -174.93
local delZAU = 37.68
local canve = '~g~[E]~w~ Parking'
local textgar = '~g~[E]~w~ Take cars ~r~(only for gold rush) ~w~'
...
Citizen.CreateThread(function()
    ...
    Citizen.Wait(10)
        ...
        if (GetDistanceBetweenCoords(coords, -173.72, 3049.86, 19.37, true) < 20.0)  then
            ...
            DrawText3D2(-173.72, 3049.86, 19.37, "~y~[E] ~b~Collect gravel")
           
            DrawText3D2(-178.4, 3044.73, 19.36, "~y~[E] ~b~Collect gravel")
          
            DrawText3D2(-182.63, 3038.37, 19.29, "~y~[E] ~b~Collect gravel")
    ...
    while true do
        ...
        if (GetDistanceBetweenCoords(coords, 283.45, 2847.5, 43.64, true) < 5.0) then
			...
			if(GetDistanceBetweenCoords(coords, 283.45, 2847.5, 43.64, true) < 3.0) then
				DrawText3D2(283.45, 2847.5, 43.64, '~y~[E] ~b~Raw gold process')
                ...
                if (hasBagd) then
                    ...
                    TriggerEvent('swt_notifications:Infos','You have no raw gold.')
                    conteggio2=0
                end
                end
            end
...
function SellKurczaki2fb()
    ...
    ----
    if(count == 0) then
    QBCore.Functions.Progressbar("search_register", "processing..", 5000, false, true, {disableMovement = true,
...
Citizen.CreateThread(function()
	...
	local s1d7 = false
	if dist <= 2.0 then
	DrawText3D2(sellX, sellY, sellZ+0.1, "~y~[E] ~b~Sell gold bars")
        ...
        TriggerEvent('swt_notifications:Infos','You have no gold bars')
        conteggio3 = 0
		end
        end	
    end
   ...
   local ad = "anim@heists@box_carry@"
	...
	if ( DoesEntityExist( player ) and not IsEntityDead( player )) then
		...
		if ( IsEntityPlayingAnim( player, ad, anim, 8 ) ) then
            ...
            TriggerEvent('swt_notifications:Infos','You got the gravel, take it to the machine.')
		end
	end
...
Citizen.CreateThread(function()
    while true do
	    ...
	    local s1d7g = false
	    if dist <= 1.0 then
	        DrawText3D2(sellXas, sellYas, sellZas+0.1, "~y~[E] ~b~Load gravel")
		         if IsControlJustPressed(0, Keys['E']) then 
if conteggio == 0 then
    conteggio = 1
                 QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
            ...
            if (hasBagd7g) then
                ...
                TriggerEvent('swt_notifications:Infos','You loaded the gravel into the machine') --success
                        ...
                        maitem = false
                        --------------------------------------Processo la sabbia
                        QBCore.Functions.Progressbar("search_register", "under processing..", 10000, false, true, {disableMovement = true,
                        disableCarMovement = true,
                        disableMouse = false,
                        disableCombat = true,
                        disableInventory = true,
                    ...
                    TriggerEvent('swt_notifications:Infos','You have no gravel .')
                    conteggio = 0
		         end
				...
				-----------eseguo il controllo se ho la carta parking----------------------
                    ...
                    ClearPedTasks(GetPlayerPed(-1))
                        ...
                        while timeLeft > 0 do
                          ...
                          TriggerServerEvent('qb-gold_rush:server:truck', model1, ClosestBerth)
                            ------------------------------------------------------------------------------------------------------
...
-----------------------------------------------GENERA AUTO----------------------------------------------
RegisterNetEvent('qb-gold_rush:Auto')
AddEventHandler('qb-gold_rush:Auto', function(boatModel, plate)
    DoScreenFadeOut(250)
    Citizen.Wait(250)
