...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
    ...
    if PlayerJob.name == "mechanic" or PlayerJob.name == "tow" then
...
AddEventHandler('jobs:client:ToggleNpc', function()
    if QBCore.Functions.GetPlayerData().job.name == "mechanic" or PlayerJob.name == "tow" then
        if CurrentTow ~= nil then 
            TriggerEvent("swt_notifications:Infos","Maak eerst je werk af!")
            return
        end
        NpcOn = not NpcOn
        if NpcOn then
        ...
        if CurrentTow == nil then 
            ...
            if NpcOn and CurrentLocation ~= nil then
                if GetEntityModel(targetVehicle) ~= GetHashKey(CurrentLocation.model) then
                    TriggerEvent("swt_notifications:Infos","This is not the right vehicle.")
                    return
                end
            end
                ...
                if vehicle ~= targetVehicle then
                    ...
                    if GetDistanceBetweenCoords(towPos.x, towPos.y, towPos.z, targetPos.x, targetPos.y, targetPos.z, true) < 11.0 then
                        QBCore.Functions.Progressbar("towing_vehicle", "Put the vehicle on it.", 5000, false, true, {
                            ...
                            if NpcOn then
                                RemoveBlip(CurrentBlip)
                                TriggerEvent("swt_notifications:Infos","Bring the vehicle to the hayens depot!")
                            end
                            TriggerEvent("swt_notifications:Infos","Vehicle lifted!")
                        end, function() -- Cancel
                            StopAnimTask(GetPlayerPed(-1), "mini@repair", "fixing_a_ped", 1.0)
            ...
            QBCore.Functions.Progressbar("untowing_vehicle", "Remove the vehicle..", 5000, false, true, {
                ...
                CurrentTow = nil
                TriggerEvent("swt_notifications:Infos","Removing the vehicle!")
            end, function() -- Cancel
                StopAnimTask(GetPlayerPed(-1), "mini@repair", "fixing_a_ped", 1.0)
                TriggerEvent("swt_notifications:Infos","Faild!")
            end)
        end
    else
        TriggerEvent("swt_notifications:Infos","You need to be in a tow truck first..")
    end
end)
                    ...
                    if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z, true) < 1.5) then
                        ...
                        if IsControlJustReleased(0, Keys["E"]) then
                            if JobsDone > 0 then
                                ...
                                TriggerEvent("swt_notifications:Infos","You haven't done any work yet..")
                            end
                        end
                ...
                if NpcOn and CurrentLocation ~= nil and next(CurrentLocation) ~= nil then
                    if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, CurrentLocation.x, CurrentLocation.y, CurrentLocation.z, true) < 50.0 and not VehicleSpawned then
                        ...
                        QBCore.Functions.SpawnVehicle(CurrentLocation.model, function(veh)
                            exports['esx_fuel']:SetFuel(veh, 0.0)
...
function deliverVehicle(vehicle)
    ...
    JobsDone = JobsDone + 1
    VehicleSpawned = false
    TriggerEvent("swt_notifications:Infos","You have delivered a vehicle!")
    TriggerEvent("swt_notifications:Infos","You can go pick up a new vehicle")
