            ...
            if Distance <= 2.0 then
                ...
                if not IsSelling then
                  ...
                  if IsControlJustReleased(0, 38) then
                      IsSelling = true
                      TriggerEvent('swt_notifications:Infos','Selling..', 'info')
...
AddEventHandler('qb-fishing:client:rod:anim', function()
    exports['nethush-assets']:AddProp('FishingRod')
    exports['nethush-assets']:RequestAnimationDict('amb@world_human_stand_fishing@idle_a')
       ...
       if Config.CanFish then
          if not IsPedInAnyVehicle(PlayerPedId()) then
           if not IsEntityInWater(PlayerPedId()) then
               ...
               TriggerEvent('swt_notifications:Infos','You are swimming.')
           end
          else
              TriggerEvent('swt_notifications:Infos','You are in a vehicle.')
          end
       else
           TriggerEvent('swt_notifications:Infos','You are not in the fishing area.')
       end
      end
  end)
end)
...
function SpawnFishBoat()
    ...
    QBCore.Functions.SpawnVehicle('dinghy', function(vehicle)
     ...
     Citizen.Wait(100)
     exports['esx_fuel']:SetFuel(vehicle, 100)
     TriggerEvent('swt_notifications:Infos','You have recieved your boat.')
