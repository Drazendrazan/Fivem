...
--QBCore.Functions.CreateCallback('qb-fishing:server:fish:reward', function()
    ...
    if RandomValue >= 100 and RandomValue < 650 then
        ...
        if SubValue == 1 then
            ...
            TriggerClientEvent('nethush-inventory:client:ItemBox', source, QBCore.Shared.Items['fish-1'], "add") 
        elseif SubValue == 2 then
            Player.Functions.AddItem('fish-2', 1)
            TriggerClientEvent('nethush-inventory:client:ItemBox', source, QBCore.Shared.Items['fish-2'], "add") 
        else
            Player.Functions.AddItem('fish-3', 1)
            TriggerClientEvent('nethush-inventory:client:ItemBox', source, QBCore.Shared.Items['fish-3'], "add") 
        end
    elseif RandomValue >= 700 and RandomValue < 820 then
        local SubValue = math.random(1,2)
        if SubValue == 1 then
            Player.Functions.AddItem('methburn', 1)
            TriggerClientEvent('nethush-inventory:client:ItemBox', source, QBCore.Shared.Items['methburn'], "add")  
        else
            Player.Functions.AddItem('plasticbag', 1)
            TriggerClientEvent('nethush-inventory:client:ItemBox', source, QBCore.Shared.Items['plasticbag'], "add") 
        end
    else
        TriggerClientEvent('swt_notifications:Infos', source, "You had nothing on your fishing hook")
    end
end)
