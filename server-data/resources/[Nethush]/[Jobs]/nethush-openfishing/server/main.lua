...
QBCore.Functions.CreateCallback('nethush-openfishing:GetItemData', function(source, cb, itemName)
	local retval = false
...
QBCore.Functions.CreateUseableItem("fishrod", function(source, item)
    ...
    TriggerClientEvent('nethush-openfishing:tryToFish', source)
end)

RegisterServerEvent('nethush-openfishing:receiveFish')
AddEventHandler('nethush-openfishing:receiveFish', function(cabin, house)
    local src = source
    ...
    if itemFound then
        for i = 1, itemCount, 1 do
            ...
            if luck == 100 then
				...
				TriggerClientEvent('nethush-inventory:client:ItemBox', src, itemInfo, "add")
...
RegisterServerEvent("nethush-openfishing:sellFish")
AddEventHandler("nethush-openfishing:sellFish", function()
    local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	local price = 0
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
		...
		TriggerClientEvent('swt_notifications:Infos', src, "You have sold your fish")
	end
end)