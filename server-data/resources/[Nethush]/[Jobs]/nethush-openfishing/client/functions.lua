TryToFish = function()
    QBCore.Functions.TriggerCallback('nethush-openfishing:GetItemData', function(count)
        if IsPedSwimming(cachedData["ped"]) then return TriggerEvent("swt_notifications:Infos","You can't be swimming and fishing at the same time.") end 
        if IsPedInAnyVehicle(cachedData["ped"]) then return TriggerEvent("swt_notifications:Infos","You need to exit your vehicle to start fishing.") end 
        if count ~= nil then
            if count == 0 then
                TriggerEvent("swt_notifications:Infos","You need both a fishing rod and bait to fish.", "primary")
            else
                ...
                if waterValidated then
                    ...
                    TriggerEvent("swt_notifications:Infos","You need to aim towards the water to fish", "primary")
                end
            end
        end
...
CastBait = function(rodHandle, castLocation)
    ...
    if not HasFishingBait() then
        TriggerEvent('swt_notifications:Infos','You don\'t have any bait!')
    ...
    while not IsControlJustPressed(0, 47) do
        ...
        if GetGameTimer() - startedCasting > 5000 then
            TriggerEvent("swt_notifications:Infos","You need to cast the bait.", "primary")
    ...
    TriggerEvent("swt_notifications:Infos","Waiting for a fish to bite...")
    TriggerServerEvent('QBCore:Server:RemoveItem', "fishingbait", 1)
	TriggerEvent("nethush-inventory:client:ItemBox", QBCore.Shared.Items["fishingbait"], "remove")
    ...
    if caughtFish then
        TriggerServerEvent("nethush-openfishing:receiveFish", castLocation, function(received) end)
        TriggerServerEvent('qb-hud:Server:RelieveStress', 1)
    else
        TriggerEvent("swt_notifications:Infos","The fish got loose.")
    end
...
IsInWater = function()
    ...
    TriggerEvent("swt_notifications:Infos","Checking fishing location...")
...
SellFish = function()
    QBCore.Functions.TriggerCallback('nethush-openfishing:GetItemData', function(count)
        ...
        TriggerServerEvent("nethush-openfishing:sellFish", function(sold, fishesSold) end)
    end)
end
