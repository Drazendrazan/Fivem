                        ...
                        if InVehicle then
                            ...
                            if IsControlJustReleased(0, Keys["E"]) then
                                QBCore.Functions.TriggerCallback('qb-garbagejob:server:CheckBail', function(DidBail)
                                    if DidBail then
                                        BringBackCar()
                                        TriggerEvent("swt_notifications:Infos","You have received a $1000 deposit back!")
                                    else
                                        TriggerEvent("swt_notifications:Infos","You have not paid a deposit on this vehicle ..")
                                    end
                                end)
                            end
                        else
                                ...
                                QBCore.Functions.TriggerCallback('qb-garbagejob:server:HasMoney', function(HasMoney)
                                    if HasMoney then
                                        ...
                                        --    exports['qb-vehiclekeys']:SetVehicleKey(GetVehicleNumberPlateText(veh), true)
                                            ...
                                            IsWorking = true
                                            SetGarbageRoute()
                                            TriggerEvent("swt_notifications:Infos","You have paid $1000!")
                                            TriggerEvent("swt_notifications:Infos","You have started working, location is indicated on your GPS!")
                                        end, coords, true)
                                    else
                                        TriggerEvent("swt_notifications:Infos","You do not have enough money for the deposit .. Deposit costs are $ 1000, -")
                                    end
                                end)
                            end
                        end
                                            ...
                                            if IsControlJustPressed(0, 51) then
                                                ...
                                                if (AmountOfBags - 1) == 0 then
                                                    ...
                                                    if currentRuns <= 2 then
                                                        SetGarbageRoute()
                                                        TriggerEvent("swt_notifications:Infos","All garbage bags are done, move on to the next location!")
                                                    else
                                                        TriggerEvent("swt_notifications:Infos","You are done working! Go back to the dump.")
                                                    ...
                                                    if AmountOfBags > 1 then
                                                        TriggerEvent("swt_notifications:Infos","There are still "..AmountOfBags.." bags left!",  "success")
                                                    else
                                                        TriggerEvent("swt_notifications:Infos","There is still "..AmountOfBags.." bag left!",  "success")
                                                    end
                                                    hasZak = false
                                                end
                                                DeliverAnim()
