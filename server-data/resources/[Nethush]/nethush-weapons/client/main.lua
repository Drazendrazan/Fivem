            ...
            if CurrentWeaponData ~= nil and next(CurrentWeaponData) ~= nil then
                if IsPedShooting(GetPlayerPed(-1)) or IsControlJustPressed(0, 24) then
                    if CanShoot then
                        ...
                        TriggerEvent('nethush-inventory:client:CheckWeapon')
                        TriggerEvent("swt_notifications:Infos","This weapon is broken and can not be used..")
                        MultiplierAmount = 0
                    end
                end
            end
            ...
            if (total + retval) <= (retval + 1) then
                QBCore.Functions.Progressbar("taking_bullets", "Loading bullets..", math.random(4000, 6000), false, true, {
                    ...
                    if QBCore.Shared.Weapons[weapon] ~= nil then
                        ...
                        TriggerEvent('nethush-inventory:client:ItemBox', QBCore.Shared.Items[itemData.name], "remove")
                        TriggerEvent('QBCore:Notify', retval.." Loading bullets!")
                    end
                end, function()
                    TriggerEvent("swt_notifications:Infos","Canceled..")
                end)
            else
                TriggerEvent("swt_notifications:Infos","Your weapon is already looded..")
            end
        else
            TriggerEvent("swt_notifications:Infos","Your not holding a weapon..")
        end
    else
        TriggerEvent("swt_notifications:Infos","Your not holding a weapon..")
    end
    ...
    if weapon ~= GetHashKey("WEAPON_UNARMED") then
        ...
        if Config.WeaponAttachments[WeaponData.name] ~= nil then
            if Config.WeaponAttachments[WeaponData.name][attachment] ~= nil then
                ...
                TriggerEvent("swt_notifications:Infos","This weapon does not support this attachment..")
            end
        end
    else
        TriggerEvent("swt_notifications:Infos","You dont have a weapon in ur hand..")
    end
end)
