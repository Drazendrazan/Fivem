...
--                 TriggerClientEvent('nethush-inventory:client:UseWeapon', src, data)
--                 TriggerClientEvent('swt_notifications:Infos', src, "Your weapon is broken, u need to repair it before u can use it again.")
--             end
--         else
            ...
            if WeaponSlot.info.quality ~= nil then
                for i = 1, RepeatAmount, 1 do
                    if WeaponSlot.info.quality - DecreaseAmount > 0 then
                        ...
                        WeaponSlot.info.quality = 0
                        TriggerClientEvent('nethush-inventory:client:UseWeapon', src, data)
                        TriggerClientEvent('swt_notifications:Infos', src, "Your weapon is broken, you need to repair it before you can use it again.")
                        break
                    end
                end
            else
...
QBCore.Functions.CreateCallback("weapons:server:RepairWeapon", function(source, cb, RepairPoint, data)
    ...
    if Player.PlayerData.items[data.slot] ~= nil then
        if Player.PlayerData.items[data.slot].info.quality ~= nil then
            if Player.PlayerData.items[data.slot].info.quality ~= 100 then
                if Player.Functions.RemoveMoney('cash', Config.WeaponRepairCotsts[WeaponClass]) then
                    ...
                    TriggerClientEvent('nethush-inventory:client:ItemBox', src, QBCore.Shared.Items[data.name], "remove")
                    TriggerClientEvent("nethush-inventory:client:CheckWeapon", src, data.name)
                ...
                TriggerClientEvent("swt_notifications:Infos", src, "This weapon is not dammaged..")
                cb(false)
            end
        else
            TriggerClientEvent("swt_notifications:Infos", src, "This weapon is not dammaged..")
        ...
        TriggerClientEvent('swt_notifications:Infos', src, "You diddent have a weapon in ur hands..")
...
AddEventHandler("weapons:server:TakeBackWeapon", function(k, data)
    ...
    TriggerClientEvent('nethush-inventory:client:ItemBox', src, QBCore.Shared.Items[itemdata.name], "add")
        ...
        if Inventory[CurrentWeaponData.slot].info.attachments ~= nil and next(Inventory[CurrentWeaponData.slot].info.attachments) ~= nil then
            ...
            if not HasAttach then
                ...
                SetTimeout(1000, function()
                    TriggerClientEvent('nethush-inventory:client:ItemBox', src, ItemData, "remove")
                end)
            else
                TriggerClientEvent("swt_notifications:Infos", src, "You already have "..AttachmentData.label:lower().." on your weapon.")
            end
        else
    ...
    if Inventory[ItemData.slot] ~= nil then
        if Inventory[ItemData.slot].info.attachments ~= nil and next(Inventory[ItemData.slot].info.attachments) ~= nil then
            ...
            if HasAttach then
                ...
                TriggerClientEvent('nethush-inventory:client:ItemBox', src, QBCore.Shared.Items[AttachmentComponent.item], "add")
                TriggerClientEvent("swt_notifications:Infos", src, "You removed "..AttachmentComponent.label.." from your weapon!")
