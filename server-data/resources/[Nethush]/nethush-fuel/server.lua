...
RegisterServerEvent("vrp_nethush-fuel2:pagamento")
AddEventHandler("vrp_nethush-fuel2:pagamento",function(price,galao,vehicle,fuel,fuel2,key)
	local source = source
	local output = {}
	local query = nil
	if key then
	...
	if price > 0 then
		...
		if galao or output.stock > amount then
			...
			if money >= price then
				xPlayer.Functions.RemoveMoney("bank", price, "Refuel-Car")
				if galao then
					TriggerClientEvent('vrp_nethush-fuel2:galao',source)
				...
				TriggerClientEvent('vrp_nethush-fuel2:insuficiente',source,vehicle,fuel2)
...
RegisterServerEvent('vrp_nethush-fuel2:LixeiroCB')
AddEventHandler('vrp_nethush-fuel2:LixeiroCB', function(key)
	local source = source
	if key then
		local sql = "SELECT stock,price FROM gas_station_business WHERE gas_station_id = @gas_station_id";
		...
		TriggerClientEvent('vrp_nethush-fuel2:LixeiroCB', source, output)
	else
		local output = {
			['price'] = Config.stock.default_price,
			['stock'] = Config.stock.default_stock,
