$(document).ready(function(){
      ...
      $(document).keyup(function(e) {
        if (!counting) {
          if (e.key === "Escape") {
            ...
            completar;
            maxFuel;
            $.post('http://nethush-fuel/escape', JSON.stringify({}));
          }
        }
      });
    } else {
  ...
  function increase() {
    if (counting) {
      if (fuel < maxFuel) {
        ...
        $.post('http://nethush-fuel/removeanim', JSON.stringify({}));
        if (completar) {
          $.post('http://nethush-fuel/pay', JSON.stringify({ new_perc: perc_new*dataGas.price }));
          perc_new;
          completar = !completar;
        } else if (guardar) {
          $.post('http://nethush-fuel/pay', JSON.stringify({ new_perc: price }));
          price;
          guardar = !guardar;
        }
        counting = !counting;
  ...
  $( "#retirar" ).click(function() {
    if (!counting) {
      ...
      $.post('http://nethush-fuel/checkpay', JSON.stringify({ new_perc: perc_new }));
      counting = !counting;
      completar = !completar;
      maxFuel = 99
    ...
    if (!counting) {
      ...
      if (price > dataGas.price) {  
        if (price <= ((100-Math.round(datafuel))*dataGas.price)) {
          $.post('http://nethush-fuel/checkpay', JSON.stringify({ new_perc: price }));
          counting = !counting;
          guardar = !guardar;
          ...
          $.post('http://nethush-fuel/notifytext', JSON.stringify({ text: texto }));
        }
      } else {
  ...
  $(".accept").click(function() {
    if (counting) {
      ...
      $('left').fadeIn(300);
      $('.act').fadeIn(300);
      $.post('http://nethush-fuel/startanim', JSON.stringify({}));
      myStop();
      myStart(); 
    }
  })
