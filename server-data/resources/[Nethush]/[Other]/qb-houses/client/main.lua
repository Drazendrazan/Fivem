...
AddEventHandler('qb-houses:client:addGarage', function()
    if closesthouse ~= nil then 
        ...
        TriggerEvent("swt_notifications:Infos","No house around..")
    end
end)
    ...
    if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, true) < 1.5)then
        if hasKey then
            if Config.Houses[closesthouse].locked then
                ...
                TriggerEvent("swt_notifications:Infos","Huis is ontgrendeld!")
            else
                TriggerServerEvent('qb-houses:server:lockHouse', true, closesthouse)
                TriggerEvent("swt_notifications:Infos","House is locked!")
            end
        else
            TriggerEvent("swt_notifications:Infos","You don't have the keys to this house...")
        end
    else
        TriggerEvent("swt_notifications:Infos","There is no door in sight??")
    end
end)
                    ...
                    if CurrentHouse ~= nil then
                        if stashLocation ~= nil then
                            if(GetDistanceBetweenCoords(pos, stashLocation.x, stashLocation.y, stashLocation.z, true) < 1.5)then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    TriggerServerEvent("nethush-inventory:server:OpenInventory", "stash", CurrentHouse)
                                    TriggerEvent("nethush-inventory:client:SetCurrentStash", CurrentHouse)
                                end
...
AddEventHandler('qb-houses:client:RingDoor', function(player, house)
    if closesthouse == house and inside then
        ...
        TriggerEvent("swt_notifications:Infos","Someone calls at the door!")
    end
end)
...
AddEventHandler('qb-houses:client:giveHouseKey', function(data)
    ...
    if player ~= -1 and distance < 2.5 and closesthouse ~= nil then
        ...
        if housedist < 10 then
            ...
            TriggerEvent("swt_notifications:Infos","You are not close enough to the house..")
        end
    elseif closesthouse == nil then
        TriggerEvent("swt_notifications:Infos","There is no house nearby!")
    else
        TriggerEvent("swt_notifications:Infos","No one around!")
    end
end)
        ...
        if housedist < 5 then
            QBCore.Functions.TriggerCallback('qb-houses:server:getHouseOwner', function(result)
                if QBCore.Functions.GetPlayerData().citizenid == result then
                    ...
                    TriggerEvent("swt_notifications:Infos","You are not a homeowner..")
                end
            end, closesthouse)
        else
            TriggerEvent("swt_notifications:Infos","You are not close enough to the house..")
...
function HouseKeysMenu()
    ...
    QBCore.Functions.TriggerCallback('qb-houses:server:getHouseKeyHolders', function(holders)
        ...
        if holders == nil or next(holders) == nil then
            TriggerEvent("swt_notifications:Infos","No key holders found..")
            closeMenuFull()
        else
...
function removeOutfit(oData)
    ...
    TriggerEvent("swt_notifications:Infos",oData.outfitname.." is removed")
    closeMenuFull()
end
...
function enterOwnedHouse(house)
    ...
    TriggerEvent('qb-weeddrug:client:getHousePlants', house)
    Citizen.Wait(100)
...
function leaveOwnedHouse(house)
    if not FrontCam then
        ...
        exports['qb-interior']:DespawnInterior(houseObj, function()
            ...
            SetEntityHeading(GetPlayerPed(-1), Config.Houses[CurrentHouse].coords.enter.h)
            TriggerEvent('qb-weeddrug:client:leaveHouse')
...
AddEventHandler('qb-houses:client:setLocation', function(data)
    ...
    if inside then
        if hasKey then
            ...
            TriggerEvent('swt_notifications:Infos','You are not the owner of the house..')
        end
    else    
        TriggerEvent('swt_notifications:Infos','You are not in a house..')
    end
end)
                        ...
                        if not Config.Houses[closesthouse].IsRaming then
                            ...
                            Skillbar.Start({
                                ...
                                if RamsDone + 1 >= Config.RamsNeeded then
                                    TriggerServerEvent('qb-houses:server:lockHouse', false, closesthouse)
                                    TriggerEvent('swt_notifications:Infos','It worked, the door is now out.')
                                ...
                                TriggerEvent('swt_notifications:Infos','It failed .. Please try again.')
                                DoRamAnimation(false)
                            end)
                            TriggerServerEvent('qb-houses:server:SetRamState', true, closesthouse)
                        else
                            TriggerEvent('swt_notifications:Infos','Someone is already working on the door..')
                        end
                    else
                        TriggerEvent('swt_notifications:Infos','This house is already open..')
                    end
                else
                    TriggerEvent('swt_notifications:Infos','You\'re not near a house..')
                end
            else
                TriggerEvent('swt_notifications:Infos','There is no police force present..')
            end
        end)
    else
        TriggerEvent('swt_notifications:Infos','You\'re not near a house..')
...
AddEventHandler('qb-houses:client:ResetHouse', function()
    ...
    if closesthouse ~= nil then
        ...
        if Config.Houses[closesthouse].IsRammed then
            ...
            TriggerServerEvent('qb-houses:server:lockHouse', true, closesthouse)
            RamsDone = 0
            TriggerEvent('swt_notifications:Infos','You locked the house again..')
        else
            TriggerEvent('swt_notifications:Infos','This door is not broken open..')
        end
    end
end)