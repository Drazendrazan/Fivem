...
AddEventHandler('qb-houses:server:addNewHouse', function(street, coords, price, tier)
	...
	TriggerClientEvent("qb-houses:client:setHouseConfig", -1, Config.Houses)
	TriggerClientEvent('swt_notifications:Infos', src, "Je hebt een huis toegevoegd: "..label)
end)
...
AddEventHandler('qb-houses:server:addGarage', function(house, coords)
	...
	TriggerClientEvent('swt_notifications:Infos', src, "You have added a garage at: "..garageInfo.label)
end)
...
AddEventHandler('qb-houses:server:buyHouse', function(house)
	...
	if (bankBalance >= HousePrice) then
		...
		TriggerClientEvent('swt_notifications:Infos', source, "You do not have enough money..")
	end
end)
...
AddEventHandler('qb-houses:server:removeHouseKey', function(house, citizenData)
	...
	TriggerClientEvent('swt_notifications:Infos', src, citizenData.firstname .. " " .. citizenData.lastname .. "'s sleutels zijn verwijderd..")
	QBCore.Functions.ExecuteSql(false, "UPDATE `player_houses` SET `keyholders` = '"..json.encode(housekeyholders[house]).."' WHERE `house` = '"..house.."'")
end)
...
AddEventHandler('qb-houses:server:giveHouseKey', function(target, house)
	...
	if tPlayer ~= nil then
		if housekeyholders[house] ~= nil then
			for _, cid in pairs(housekeyholders[house]) do
				if cid == tPlayer.PlayerData.citizenid then
					TriggerClientEvent('swt_notifications:Infos', src, 'This person already has the keys to this house!')
					return
				end
			end		
			...
			TriggerClientEvent('swt_notifications:Infos', tPlayer.PlayerData.source, 'You have the keys to '..Config.Houses[house].adress..' !')
		else
		...
		TriggerClientEvent('swt_notifications:Infos', src, 'Something went wrong .. Please try again!')
	end
end)
...
QBCore.Functions.CreateUseableItem("police_stormram", function(source, item)
	...
	if (Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty) then
		...
		TriggerClientEvent('swt_notifications:Infos', source, "This is only possible for emergency services!")
	end
end)
