...
AddEventHandler('coke:onUse', function()
	if Config.useMythic then
		TriggerEvent("swt_notifications:Infos","You used Coke")
	end
	...
	DoScreenFadeIn(2000)
	if Config.useMythic then
		TriggerEvent("swt_notifications:Infos","You are feeling normal now..")
	end
...
function main()
	...
	if Config.useMythic then
		TriggerEvent("swt_notifications:Infos","Go to the airfield.")
	end
	...
	while checkPlane do
		...
		if DoesEntityExist(airplane) then
			if GetVehicleEngineHealth(airplane) < 0 then
				if Config.useMythic then
					TriggerEvent("swt_notifications:Infos","Failed, your plane was Destroyed")
				end
				TriggerServerEvent('coke:updateTable', false)
				RemoveBlip(blip)
...
function delivery()
	if Config.useMythic then
		TriggerEvent("swt_notifications:Infos","Get in the plane and pick up the delivery marked on your GPS")
	end
		...
		while delivering do
			...
			if disttocoord <= 30 then
				...
				if IsControlJustPressed(1, 51) then
					...
					Citizen.Wait(2000)
					TriggerEvent("swt_notifications:Infos","Picked up the delivery. Return to the airfield marked on your GPS.")
					Citizen.Wait(2000)
					final()
				end
			else
...
function final()
	TriggerEvent("swt_notifications:Infos","Deliver the plane back to a hangar")
			...
			if IsControlJustPressed(1, 51) then		
				...
				QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
					if result then
						...
						TriggerEvent("swt_notifications:Infos","You need coke brick to do this")
						isProcessing = false
					end
				end, 'coke_brick')
			end
