	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
		...
		if success then
			...
			if OxyChance <= Config.OxyChance then
				QBCore.Functions.TriggerCallback('oxydelivery:server:receiveOxey', function()
				end)
			else
				QBCore.Functions.TriggerCallback('oxydelivery:server:receiveMoney', function()
				end)
			end

			Citizen.Wait(2000)
			TriggerEvent("swt_notifications:Infos","Delivery was good, your pager will be updated with the next drop-off")
		else
			TriggerEvent("swt_notifications:Infos","Delivery has failed")
		end
...
AddEventHandler("oxydelivery:client", function()
	...
	while tasking do
		...
		if dstcheck < 40.0 and not pedCreated and (oxyVehicle == veh or dstcheck2 < 15.0) then
			...
			DeleteCreatedPed()
			CreateOxyPed()
			TriggerEvent("swt_notifications:Infos","You are close to the drop-off")
		end
		toolong = toolong - 1
		if toolong < 0 then
		    SetVehicleHasBeenOwnedByPlayer(oxyVehicle,false)
			...
			tasking = false
			OxyRun = false
			TriggerEvent("swt_notifications:Infos","You are done delivering, it is taking too long!")
			--TriggerClientEvent('swt_notifications:Infos', source, "Je verkoopt geen oxy meer omdat het lang duurt")
		end
		if dstcheck < 2.0 and pedCreated then
			local crds = GetEntityCoords(deliveryPed)
			DrawText3Ds(crds["x"],crds["y"],crds["z"], "[E] Dealing")  
		...
		if QBCore ~= nil then
			...
			if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations["job"].x, Config.Locations["job"].y, Config.Locations["job"].z, true) < 1.5) then
            	...
            	if hours >= Config.MinimumTime and hours <= Config.MaximumTime then
					QBCore.Functions.DrawText3D(Config.Locations["job"].x, Config.Locations["job"].y, Config.Locations["job"].z, "~g~[E]~w~ $1500 - Deliver Job (Beloning Cash + Oxy)")
					if IsControlJustReleased(0, Keys["E"]) then
						if CurrentCops >= Config.MinimumOxyPolice then
							TriggerServerEvent("oxydelivery:server")
							Citizen.Wait(1000)
						else
							TriggerEvent('swt_notifications:Infos','Not enough agents..')
						end
					end
				else
					QBCore.Functions.DrawText3D(Config.Locations["job"].x, Config.Locations["job"].y, Config.Locations["job"].z, "Pharmacy is closed (9:00 AM to 7:00 PM)")
				end	
			end
		else
            Citizen.Wait(1000)
    ...
    while true do
		...
		elseif OxyRun then
			if not DoesEntityExist(oxyVehicle) or GetVehicleEngineHealth(oxyVehicle) < 200.0 or GetVehicleBodyHealth(oxyVehicle) < 200.0 then
				OxyRun = false
				tasking = false
				TriggerEvent("swt_notifications:Infos","The dealer will no longer give you a location because your car is damaged")
			else
				if tasking then
			        Citizen.Wait(20000)
			    else
				    ...
				    if salecount == Config.RunAmount then
				    	Citizen.Wait(30000)
						OxyRun = false
						TriggerEvent("swt_notifications:Infos","You are done delivering, go back to the pharmacy!")
				    end
				end
			end
	    else
...
RegisterNetEvent('qb-client:client:executeOxyEvents')
AddEventHandler('qb-client:client:executeOxyEvents', function()
...
function doPoliceAlert()
    ...
    TriggerServerEvent('qb-oxyruns:server:callCops', streetLabel, pos)
end

RegisterNetEvent('qb-oxyruns:client:robberyCall')
AddEventHandler('qb-oxyruns:client:robberyCall', function(msg, streetLabel, coords)
	PlaySound(-1, "Lose_1st", "GTAO_FM_Events_Soundset", 0, 0, 1)
	TriggerEvent('qb-policealerts:client:AddPoliceAlert', {
		timeOut = 5000,
		alertTitle = "Illegaal Drugs Trade",
		coords = {
			x = coords.x,
			y = coords.y,
			z = coords.z,
