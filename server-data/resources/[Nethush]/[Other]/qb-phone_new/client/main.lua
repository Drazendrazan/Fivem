        ...
        if IsControlJustPressed(0, 244) then
            if not PhoneData.isOpen then
                ...
                if not IsHandcuffed then
                    ...
                    TriggerEvent("swt_notifications:Infos","Action currently not possible..")
                end
            end
        end
        Citizen.Wait(3)
...
function LoadPhone()
    ...
    QBCore.Functions.TriggerCallback('qb-phone_new:server:GetPhoneData', function(pData)
        ...
        print("Self Tweets Length 2 " .. #PhoneData.SelfTweets .. " Tweets Length 2" .. #PhoneData.Tweets)
...
function OpenPhone()
    QBCore.Functions.TriggerCallback('qb-phone_new:server:HasPhone', function(HasPhone)
        if HasPhone then
            ...
            TriggerEvent("swt_notifications:Infos","You don't have a phone")
        end
    end)
end
...
RegisterNUICallback('DeclineInvoice', function(data, cb)
    TriggerEvent("debug", 'Phone: Decline Invoice')
...
--     TriggerServerEvent("qb_phone:deleteTweet", data.id)
...
--     TriggerServerEvent('qb_phone:server:updateForEveryone', PhoneData.Tweets)
-- end)
...
while takePhoto do
    ...
    exports['screenshot-basic']:requestScreenshotUpload('https://discord.com/api/webhooks/971971128371327047/OP2DIosneAsnN3Pzp4jO06GrflFRzUIfyPqz8vRqByM5BKnYOpsf8YFYBrggRz_-SPLb', 'files[]', function(data2)
        DestroyMobilePhone()
...
function AnswerCall()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        exports["mumble-voip"]:SetCallChannel(PhoneData.CallData.CallId, 'Telefoon')
...
RegisterNUICallback('SetAlertWaypoint', function(data)
    ...
    TriggerEvent('swt_notifications:Infos','GPS Locatie ingesteld: '..data.alert.title)
...
RegisterNetEvent('qb-phone_new:client:addBankMoney')
AddEventHandler('qb-phone_new:client:addBankMoney', function(amount)
    if PhoneData.isOpen then
        SendNUIMessage({
            action = "PhoneNotification",
            PhoneNotify = {
                title = "Bank",
                text = "LKR"..amount..",- Added from your bank!", 
                icon = "fas fa-university", 
                color = "#03e972",
                timeout = 3500,
            },
        ...
        TriggerServerEvent("InteractSound_SV:PlayOnSource", "ringing2", 0.95)
        SendNUIMessage({
            action = "Notification",
            NotifyData = {
                title = "Bank",
                content = "LKR"..amount..",- Added from your bank!", 
                icon = "fas fa-university",
                timeout = 5500, 
                color = "#03e972",
            },
...
AddEventHandler('qb-phone_new:client:RemoveBankMoney', function(amount)
    if PhoneData.isOpen then
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Bank",
                text = "LKR"..amount..",- debited from your bank!", 
                icon = "fas fa-university", 
                color = "#ff002f",
                timeout = 3500,
            },
        ...
        TriggerServerEvent("InteractSound_SV:PlayOnSource", "that", 0.25)
        SendNUIMessage({
            action = "Notification",
            NotifyData = {
                title = "Bank",
                content = "LKR"..amount..",- debited from your bank!", 
                icon = "fas fa-university",
                timeout = 3500, 
                color = "#ff002f",
            },
...
RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    QBCore.Functions.TriggerCallback('qb-lapraces:server:GetRacingData', function(RaceData)
        ...
        if dist <= 115.0 then
            ...
            TriggerEvent('swt_notifications:Infos','You are too far from the race. Your navigation is set to the race.')
...
RegisterNUICallback('SetGPSLocation', function(data, cb)
    ...
    TriggerEvent('swt_notifications:Infos','GPS is set!')
end)
...
RegisterNUICallback('GetCurrentMechanic', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('qb-phone_new:server:GetCurrentMechanic', function(mechanic)
        cb(mechanic)
    end)
end)
...
RegisterNUICallback("GetCurrentDoctor", function(data, cb)
    QBCore.Functions.TriggerCallback('qb-phone_new:server:GetCurrentDoctor', function(drivers)
        cb(drivers)
    end)
end)