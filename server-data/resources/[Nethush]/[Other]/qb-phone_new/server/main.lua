...
-- RegisterServerEvent('qb_phone:deleteTweet')
-- AddEventHandler('qb_phone:deleteTweet', function(id)
...
-- RegisterServerEvent('qb_phone:saveTwitterToDatabase')
-- AddEventHandler('qb_phone:saveTwitterToDatabase', function(firstName, lastname, message, url, time, picture)
--     local xPlayer = QBCore.Functions.GetPlayer(source)
...
RegisterServerEvent('qb_phone:server:updateForEveryone')
AddEventHandler('qb_phone:server:updateForEveryone', function(newTweet)
    local src = source
    print('has work')
    TriggerClientEvent('qb_phone:updateForEveryone', -1, newTweet)
end)

RegisterServerEvent('qb_phone:server:updateidForEveryone')
AddEventHandler('qb_phone:server:updateidForEveryone', function()
    TriggerClientEvent('qb_phone:updateidForEveryone', -1)
end)
...
AddEventHandler('qb-phone_new:server:TransferMoney', function(iban, amount)
    ...
    QBCore.Functions.ExecuteSql(false, "SELECT * FROM `players` WHERE `charinfo` LIKE '%"..iban.."%'", function(result)
        if result[1] ~= nil then
            ...
            TriggerClientEvent('swt_notifications:Infos', src, "This account number does not exist!")
        end
    end)
end)
...
QBCore.Functions.CreateCallback('qb-phone_new:server:ScanPlate', function(source, cb, plate)
    ...
    if plate ~= nil then 
        ...
        TriggerClientEvent('swt_notifications:Infos', src, "No vehicle around..")
        cb(nil)
    end
end)
...
QBCore.Functions.CreateCallback('qb-phone_new:server:GetCurrentpolices', function(source, cb)
    ...
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        local Player = QBCore.Functions.GetPlayer(v)
        if Player ~= nil then
            if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "police8" or Player.PlayerData.job.name == "police7" or Player.PlayerData.job.name == "police6" or Player.PlayerData.job.name == "police5" or Player.PlayerData.job.name == "police4" or Player.PlayerData.job.name == "police3" or Player.PlayerData.job.name == "police2" or Player.PlayerData.job.name == "police1" then
...
QBCore.Functions.CreateCallback('qb-phone_new:server:GetCurrentDoctor', function(source, cb)
    local doctor = {}
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        local Player = QBCore.Functions.GetPlayer(v)
        if Player ~= nil then
            if Player.PlayerData.job.name == "doctor" or Player.PlayerData.job.name == "ambulance" or Player.PlayerData.job.name == "ambulance1" or Player.PlayerData.job.name == "ambulance" or Player.PlayerData.job.name == "ambulance4" then
                table.insert(doctor, {
                    name = Player.PlayerData.charinfo.firstname .. " " .. Player.PlayerData.charinfo.lastname,
                    phone = Player.PlayerData.charinfo.phone,
                })
...
QBCore.Functions.CreateCallback('qb-phone_new:server:GetCurrentMechanic', function(source, cb)
    local Mechanic = {}
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        local Player = QBCore.Functions.GetPlayer(v)
        if Player ~= nil then
...
QBCore.Functions.CreateCallback('qb-phone_new:server:GetCurrentDrivers', function(source, cb)
 ...
 QBCore.Commands.Add("fine", "Bill Player", {{name="id", help="Player ID"}, {name="amount", help="Fine Amount"}, {name="title", help="Fine Title"}}, false, function(source, args)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
     ...
     if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "police8" or Player.PlayerData.job.name == "police7" or Player.PlayerData.job.name == "police6" or Player.PlayerData.job.name == "police5" or Player.PlayerData.job.name == "police4" or Player.PlayerData.job.name == "police3" or Player.PlayerData.job.name == "police2" or Player.PlayerData.job.name == "police1" or Player.PlayerData.job.name == 'ambulance' or Player.PlayerData.job.name == 'mechanic' or Player.PlayerData.job.name == 'cardealer' then
         if OtherPlayer ~= nil then
             if price and price > 0 then
                 QBCore.Functions.ExecuteSql(false, "INSERT INTO `phone_invoices` (`citizenid`, `amount`, `society`, `title`) VALUES ('" .. OtherPlayer.PlayerData.citizenid .. "', " .. price .. ", '" .. Player.PlayerData.job.name .. "', '" .. title .. "')", function()
                     TriggerClientEvent('swt_notifications:Infos', OtherPlayer.PlayerData.source, "You received a fine for a total of $" .. price)
