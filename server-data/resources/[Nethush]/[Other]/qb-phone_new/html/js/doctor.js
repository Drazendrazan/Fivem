SetupDoctor = function(data) {
    ...
    if (data.length > 0) {
        $.each(data, function(i, ambulance){
            var element = '<div class="doctor-list" id="doctorid-'+i+'"> <div class="doctor-list-firstletter">' + (ambulance.name).charAt(0).toUpperCase() + '</div> <div class="doctor-list-fullname">' + ambulance.name + '</div> <div class="doctor-list-call"><i class="fas fa-phone"></i></div> </div>'
            $(".doctor2-list").append(element);
            $("#doctorid-"+i).data('doctorData', ambulance);
        });
    } else {
