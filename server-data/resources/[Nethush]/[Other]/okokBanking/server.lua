...
AddEventHandler("okokBanking:DepositMoney", function(amount)
	...
	if amount <= playerMoney then
		...
		TriggerClientEvent('okokNotify:Alert', _source, "BANK", "You don't have that much money on you", 5000)
	end
end)
...
AddEventHandler("okokBanking:WithdrawMoney", function(amount)
	...
	if amount <= playerMoney then
		...
		TriggerClientEvent('okokNotify:Alert', _source, "BANK", "You don't have that much money on the bank", 5000)
	end
end)
...
AddEventHandler("okokBanking:TransferMoney", function(amount, ibanNumber, targetIdentifier, acc, targetName)
	...
	if xPlayer.PlayerData.citizenid ~= targetIdentifier then
		...
		TriggerClientEvent('okokNotify:Alert', _source, "BANK", "You can't send money to yourself", 5000)
	end
end)
...
AddEventHandler("okokBanking:WithdrawMoneyToSociety", function(amount, society, societyName, societyMoney)
	...
	if amount <= db.value then
		if db.is_withdrawing == 1 then
			TriggerClientEvent('okokNotify:Alert', _source, "BANK", "Someone is already withdrawing", 5000)
		else
		...
		TriggerClientEvent('okokNotify:Alert', _source, "BANK", "Your society doesn't have that much money on the bank", 5000)
	end
...
AddEventHandler("okokBanking:UpdateIbanDB", function(iban, amount)
	...
	if amount <= xPlayer.PlayerData.money.bank then
		...
		TriggerClientEvent('okokNotify:Alert', _source, "BANK", "You need to have "..amount.."€ in order to change your IBAN", 5000)
	end
end)
...
AddEventHandler("okokBanking:UpdatePINDB", function(pin, amount)
	...
	if amount <= xPlayer.PlayerData.money.bank then
		...
		TriggerClientEvent('okokNotify:Alert', _source, "BANK", "You need to have "..amount.."€ in order to change your PIN", 5000)
	end
end)