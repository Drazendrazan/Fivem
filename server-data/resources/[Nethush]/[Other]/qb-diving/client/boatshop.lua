    ...
    while true do
        ...
        if distance < 15 then
            ...
            if BuyDistance < 2 then                
                ...
                DrawMarker(35, QBBoatshop.Locations["berths"][ClosestBerth]["coords"]["boat"]["x"], QBBoatshop.Locations["berths"][ClosestBerth]["coords"]["boat"]["y"], QBBoatshop.Locations["berths"][ClosestBerth]["coords"]["boat"]["z"] + 1.5, 0.0, 0.0, 0.0, 0.0, 5.0, 1.0, 1.0, 0.1, -0.8, 15, 255, 55, 255, true, false, false, true, false, false, false)

                if not Buying then
                    DrawText3D(BuyLocation.x, BuyLocation.y, BuyLocation.z + 0.3, '~b~E~w~ - Buy '..QBBoatshop.ShopBoats[currentBoat]["label"]..' for ~g~$'..QBBoatshop.ShopBoats[currentBoat]["price"])
                    if IsControlJustPressed(0, Keys["E"]) then
                        Buying = true
                    end
                else
                    DrawText3D(BuyLocation.x, BuyLocation.y, BuyLocation.z + 0.3, 'Are you sure? ~g~7~w~ YES! / ~r~8~w~ Nope! ~b~($'..QBBoatshop.ShopBoats[currentBoat]["price"]..')')
                    if IsControlJustPressed(0, Keys["7"]) or IsDisabledControlJustReleased(0, Keys["7"]) then
                        TriggerServerEvent('cash-divingjob:server:BuyBoat', QBBoatshop.Locations["berths"][ClosestBerth]["boatModel"], ClosestBerth)
                        Buying = false
...
RegisterNetEvent('cash-divingjob:client:BuyBoat')
AddEventHandler('cash-divingjob:client:BuyBoat', function(boatModel, plate)
    DoScreenFadeOut(250)
    Citizen.Wait(250)
