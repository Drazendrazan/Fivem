...
RegisterNetEvent('cash-divingjob:client:NewLocations')
AddEventHandler('cash-divingjob:client:NewLocations', function()
    QBCore.Functions.TriggerCallback('cash-divingjob:server:GetDivingConfig', function(Config, Area)
        QBDiving.Locations = Config
        TriggerEvent('cash-divingjob:client:SetDivingLocation', Area)
    end)
end)

RegisterNetEvent('cash-divingjob:client:SetDivingLocation')
AddEventHandler('cash-divingjob:client:SetDivingLocation', function(DivingLocation)
...
function TakeCoral(coral)
    ...
    TriggerServerEvent('cash-divingjob:server:TakeCoral', CurrentDivingLocation.Area, coral, true)
end

RegisterNetEvent('cash-divingjob:client:UpdateCoral')
AddEventHandler('cash-divingjob:client:UpdateCoral', function(Area, Coral, Bool)
...
function CallCops()
    ...
    if Call == Chance then
        TriggerServerEvent('cash-divingjob:server:CallCops', Coords)
    end
end

RegisterNetEvent('cash-divingjob:server:CallCops')
AddEventHandler('cash-divingjob:server:CallCops', function(Coords, msg)
...
RegisterNetEvent('cash-divingjob:client:UseGear')
AddEventHandler('cash-divingjob:client:UseGear', function(bool)
    if bool then
        GearAnim()
        QBCore.Functions.Progressbar("equip_gear", "Putting on wet suit..", 5000, false, true, {}, {}, {}, {}, function() -- Done
            ...
            TriggerServerEvent('cash-divingjob:server:RemoveGear')
        ...
        if currentGear.enabled then
            ...
            QBCore.Functions.Progressbar("remove_gear", "Removing wet suit..", 5000, false, true, {}, {}, {}, {}, function() -- Done
                ...
                TriggerServerEvent('cash-divingjob:server:GiveBackGear')
                ClearPedTasks(GetPlayerPed(-1))
                TriggerEvent('swt_notifications:Infos','You took your wetsuit off')
            end)
        else
            TriggerEvent('swt_notifications:Infos','You are not wearing a diving gear..')
        end
    end
end)
