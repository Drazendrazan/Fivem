    ...
    if RepairPart ~= nil then
        if RepairPart.amount >= Config.RepairCostAmount[part].costs then
            ...
            for i = 1, Config.RepairCostAmount[part].costs, 1 do
                TriggerClientEvent('nethush-inventory:client:ItemBox', src, QBCore.Shared.Items[Config.RepairCostAmount[part].item], "remove")
                Citizen.Wait(500)
            end
        else
            TriggerClientEvent('swt_notifications:Infos', src, "Je hebt niet genoeg "..QBCore.Shared.Items[Config.RepairCostAmount[part].item]["label"].." (min. "..Config.RepairCostAmount[part].costs.."x)")
        end
    else
        TriggerClientEvent('swt_notifications:Infos', src, "Je hebt geen "..QBCore.Shared.Items[Config.RepairCostAmount[part].item]["label"].." bij je!")
    end
end)
    ...
    if IsAuthorized(Player.PlayerData.citizenid) then
        ...
        if TargetId ~= nil then
            ...
            if TargetData ~= nil then
                ...
                TriggerClientEvent('swt_notifications:Infos', TargetData.PlayerData.source, "Je aangenomen als Autocare medewerker!")
                TriggerClientEvent('swt_notifications:Infos', source, "Je hebt ("..TargetData.PlayerData.charinfo.firstname..") aangenomen als Autocare medewerker!")
            end
        else
            TriggerClientEvent('swt_notifications:Infos', source, "Je moet wel een Speler ID meegeven!")
        end
    else
        TriggerClientEvent('swt_notifications:Infos', source, "Je kan dit niet doen!") 
    end
end)
        ...
        if TargetId ~= nil then
            ...
            if TargetData ~= nil then
                if TargetData.PlayerData.job.name == "mechanic" then
                    ...
                    TriggerClientEvent('swt_notifications:Infos', TargetData.PlayerData.source, "Je bent ontslagen als Autocare medewerker!")
                    TriggerClientEvent('swt_notifications:Infos', source, "Je hebt ("..TargetData.PlayerData.charinfo.firstname..") ontslagen als Autocare medewerker!")
                else
                    TriggerClientEvent('swt_notifications:Infos', source, "Dit is geen medewerker van Autocare!")
                end
            end
        else
            TriggerClientEvent('swt_notifications:Infos', source, "Je moet wel een Speler ID meegeven!")
