		    ...
		    if GetPedInVehicleSeat(vehicled, -1) == GetPlayerPed(-1) then
                ...
                if IsControlJustPressed(0, Keys['E']) then 
                    if lavorare == false then
                        ...
                        TriggerEvent('swt_notifications:Infos','Knock Knock!')
...
AddEventHandler('open:market', function()
    ...
    SetWeaponSeries()
    TriggerServerEvent("nethush-inventory:server:OpenInventory", "shop", "Market", Config.Itemspistol)
    lavorare = false
end)
...
AddEventHandler('open:marketGang', function()
    ...
    SetWeaponSeries()
    TriggerServerEvent("nethush-inventory:server:OpenInventory", "shop", "Market", Config.Items)
    lavorare = false
end)
-------------------------------------------------------------------------------------------------------------------------------------
        ...
        if dist <= 3.0 then
		    ...
		    if GetPedInVehicleSeat(vehicled, -1) == GetPlayerPed(-1) then
              ----se sono in macchina non esegue nessuna funzione
                ...
                if IsControlJustPressed(0, Keys['E']) then 
                    TriggerEvent("nethush-inventory:client:SetCurrentStash", "gangarmory")
                    TriggerServerEvent("nethush-inventory:server:OpenInventory", "stash", "gangarmory", {
                        maxweight = 4000000,
                        slots = 500,
                    })
        ...
        if dist <= 3.0 then
		    ...
		    if GetPedInVehicleSeat(vehicled, -1) == GetPlayerPed(-1) then
              ----se sono in macchina non esegue nessuna funzione
                ...
                if IsControlJustPressed(0, Keys['E']) then 
                    TriggerEvent("nethush-inventory:client:SetCurrentStash", "nomegang")
                    TriggerServerEvent("nethush-inventory:server:OpenInventory", "stash", "nomegang", {
                        maxweight = 4000000,
                        slots = 500,
                    })
