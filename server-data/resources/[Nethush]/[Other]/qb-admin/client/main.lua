...
RegisterNetEvent('QBCore:ClearArea')
AddEventHandler('QBCore:ClearArea', function(x, y, z)
...
AddEventHandler('qb-admin:client:openMenu', function(group)
    QBCore.Functions.TriggerCallback('qb-admin:server:hasPermissions', function(res)
        if res then
            WarMenu.OpenMenu('admin')
            myPermissionRank = group
            DealersData = dealers
        else
            --TriggerEvent('swt_notifications:Infos','You dont have the rights for this')
            TriggerEvent("swt_notifications:Negative","SLMC System","You dont have the rights for this","right",5000,true)
        end
    end, group)
end)
...
Citizen.CreateThread(function()
    ...
    WarMenu.CreateMenu('admin', 'SLMC Admin Menu')
    ...
    while true do
        ...
        elseif WarMenu.IsMenuOpened('selfOptions') then
            if WarMenu.CheckBox("Noclip", isNoclip, function(checked) isNoclip = checked end) then
                ...
                QBCore.Functions.TriggerCallback('qb-admin:server:togglePlayerNoclip', function(result)
                end, targetId)
            ...
            if WarMenu.Button('Revive') then
                local target = PlayerId()
                local targetId = GetPlayerServerId(target)
                QBCore.Functions.TriggerCallback('qb-admin:server:revivePlayer', function(result)
                end, targetId)
        ...
        elseif WarMenu.IsMenuOpened('recording') then
            if WarMenu.MenuButton('Start Recording', 'recording') then
                if (IsRecording()) then
                   -- TriggerEvent('swt_notifications:Infos','You are already recording.')
                    TriggerEvent("swt_notifications:Warning","SLMC System","You are already recording.","top-right",5000,true)
                else
                    StartRecording(1)
                end
            end
            if WarMenu.MenuButton('Stop Recording', 'recording') then
                if not (IsRecording()) then
                  --  TriggerEvent('swt_notifications:Infos','You are not recording.')
                    TriggerEvent("swt_notifications:Warning","SLMC System",'You are not recording.',"top-right",4000,true)
                else
        ...
        elseif WarMenu.IsMenuOpened('playerOptions') then
            ...
            -- QBCore.Functions.TriggerCallback('qb-admin:server:getTargetData', function(bsnr, fnamer, lnamer)
            --     if WarMenu.MenuButton('BSN: '..bsn, currentPlayer) then
            --         TriggerEvent('swt_notifications:Infos','Naam: '..fname.." "..lname)
            --     end
            -- end, target)
            
            if WarMenu.MenuButton('Kill', currentPlayer) then
                QBCore.Functions.TriggerCallback('qb-admin:server:killPlayer', function(result)
            ...
            if WarMenu.CheckBox("Freeze", isFreeze, function(checked) isFreeze = checked end) then
                local target = GetPlayerServerId(currentPlayer)
                QBCore.Functions.TriggerCallback('qb-admin:server:Freeze', function(result)
                end, target, isFreeze)
            ...
            if WarMenu.MenuButton("Give Clothing Menu", currentPlayer) then
                ...
                QBCore.Functions.TriggerCallback('qb-admin:server:OpenSkinMenu', function(result)
                end, targetId)
        ...
        elseif WarMenu.IsMenuOpened('teleportOptions') then
            ...
            if WarMenu.MenuButton('Bring', currentPlayer) then
                ...
                QBCore.Functions.TriggerCallback('qb-admin:server:bringTp', function(result)
        ...
        elseif WarMenu.IsMenuOpened('permissionOptions') then
            if WarMenu.ComboBox('Permission Group', perms, currentPermIndex, selectedPermIndex, function(currentIndex, selectedIndex)
                ...
                QBCore.Functions.TriggerCallback('qb-admin:setPermissions', function(result)
              --  TriggerEvent('swt_notifications:Infos','You have '..GetPlayerName(currentPlayer)..'\' group has changed to '..group.label)
                TriggerEvent("swt_notifications:Success","SLMC System",'You have '..GetPlayerName(currentPlayer)..'\' group has changed to '..group.label,"right",6000,true)
        ...
        elseif WarMenu.IsMenuOpened('adminOptions') then
            if WarMenu.ComboBox('Ban Length', bans, currentBanIndex, selectedBanIndex, function(currentIndex, selectedIndex)
                ...
                if reason ~= nil and reason ~= "" and time ~= 0 then
                    local target = GetPlayerServerId(currentPlayer)
                    QBCore.Functions.TriggerCallback('qb-admin:server:banPlayer', function(result)
            ...
            if WarMenu.MenuButton('Kick', currentPlayer) then
                ...
                if reason ~= nil and reason ~= "" then
                    local target = GetPlayerServerId(currentPlayer)
                    QBCore.Functions.TriggerCallback('qb-admin:server:kickPlayer', function(result)
                    end, target, reason)
        ...
        elseif WarMenu.IsMenuOpened('weatherOptions') then
            for k, v in pairs(AvailableWeatherTypes) do
                if WarMenu.MenuButton(AvailableWeatherTypes[k].label, 'weatherOptions') then
                 ...
                 --   TriggerEvent('swt_notifications:Infos','Again has changed to: '..AvailableWeatherTypes[k].label)
                    TriggerEvent("swt_notifications:Warning","SLMC System",'Again has changed to: '..AvailableWeatherTypes[k].label,"right",6000,true)
                end
            end
        ...
        elseif WarMenu.IsMenuOpened('dealerManagement') then
            ...
            elseif WarMenu.IsMenuOpened('createDealer') then
                if WarMenu.ComboBox('Min. Time', times, currentMinTimeIndex, selectedMinTimeIndex, function(currentIndex, selectedIndex)
                  ...
                  --  TriggerEvent('swt_notifications:Infos','Time confirmed!')
                    TriggerEvent("swt_notifications:Success","SLMC System",'Time confirmed!',"top-right",6000,true)
                end
...
function OpenTargetInventory(targetId)
    ...
    TriggerServerEvent("nethush-inventory:server:OpenInventory", "otherplayer", targetId)
end

function OpenTargetAppartmentInventory(target)
    WarMenu.CloseMenu()
    
    --TriggerServerEvent("nethush-inventory:server:OpenInventory", "stash", target)
end
...
AddEventHandler('qb-admin:client:SaveCar', function()
    ...
    if veh ~= nil and veh ~= 0 then
        ...
        if QBCore.Shared.VehicleModels[hash] ~= nil and next(QBCore.Shared.VehicleModels[hash]) ~= nil then
            QBCore.Functions.TriggerCallback('qb-admin:server:SaveCar', function(result)
            end, props, QBCore.Shared.VehicleModels[hash], GetHashKey(veh), plate)
            -- TriggerServerEvent('qb-admin:server:SaveCar', props, QBCore.Shared.VehicleModels[hash], GetHashKey(veh), plate)
        else
          --  TriggerEvent('swt_notifications:Infos','You cannot put this vehicle in your garage..')
            TriggerEvent("swt_notifications:Negative","SLMC System",'You cannot put this vehicle in your garage..',"right",8000,true)
        end
    else
       -- TriggerEvent('swt_notifications:Infos','You are not in a vehicle..')
       TriggerEvent("swt_notifications:Negative","SLMC System",'You are not in a vehicle..',"right",8000,true)
    end
end)
...
RegisterNetEvent('qb-admin:client:executeEvents')
AddEventHandler('qb-admin:client:executeEvents', function()
...
AddEventHandler('qb-weapons:client:SetWeaponAmmoManual', function(weapon, ammo)
    ...
    if weapon ~= "current" then
        ...
        --TriggerEvent('swt_notifications:Infos','+'..ammo..' Ammo for the '..QBCore.Shared.Weapons[GetHashKey(weapon)]["label"])
        TriggerEvent("swt_notifications:Info","SLMC System",'+'..ammo..' Ammo for the '..QBCore.Shared.Weapons[GetHashKey(weapon)]["label"],"right",8000,true)
    else
        local weapon = GetSelectedPedWeapon(ped)
        if weapon ~= nil then
            SetPedAmmo(ped, weapon, ammo)
            --TriggerEvent('swt_notifications:Infos','+'..ammo..' Ammo for the '..QBCore.Shared.Weapons[weapon]["label"])
            TriggerEvent("swt_notifications:Info","SLMC System",'+'..ammo..' Ammo for the '..QBCore.Shared.Weapons[weapon]["label"],"right",8000,true)
        else
           -- TriggerEvent('swt_notifications:Infos','You are not holding a weapon..')
            TriggerEvent("swt_notifications:Negative","SLMC System",'You are not holding a weapon..',"right",6000,true)
        end
    end
end)
