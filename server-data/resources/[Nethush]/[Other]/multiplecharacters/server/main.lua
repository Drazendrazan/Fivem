...
AddEventHandler('qb-multicharacter:server:disconnect', function()
    ...
    DropPlayer(src, "You have disconnected from Roleplay")
end)
...
AddEventHandler('qb-multicharacter:server:loadUserData', function(cData)
    ...
    if QBCore.Player.Login(src, cData.citizenid) then
        print('^2[nethush-core]^7 '..GetPlayerName(src)..' (Citizen ID: '..cData.citizenid..') has succesfully loaded!')
...
AddEventHandler('qb-multicharacter:server:createCharacter', function(data)
    ...
    if QBCore.Player.Login(src, false, newData) then
        print('^2[un-core]^7 '..GetPlayerName(src)..' has succesfully loaded!')
        QBCore.Commands.Refresh(src)
        loadHouseData()
...
QBCore.Commands.Add("char", "Give the character menu to a player", {{name="id", help="Player ID"}}, false, function(source, args)
