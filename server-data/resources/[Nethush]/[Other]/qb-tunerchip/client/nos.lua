    ...
    if not NitrousActivated then
        if IsInVehicle and not IsThisModelABike(GetEntityModel(GetVehiclePedIsIn(ped))) then
            QBCore.Functions.Progressbar("use_nos", "NOS is getting connected.", 1000, false, true, {
                ...
                TriggerEvent("nethush-inventory:client:ItemBox", QBCore.Shared.Items['nitrous'], "remove")
            ...
            TriggerEvent('swt_notifications:Infos','You are not in vehicle.')
        end
    else
        TriggerEvent('swt_notifications:Infos','You already have NOS active..')
    end
end)
