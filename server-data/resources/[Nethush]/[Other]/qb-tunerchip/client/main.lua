...
RegisterNUICallback('save', function(data)
    QBCore.Functions.TriggerCallback('qb-tunerchip:server:HasChip', function(HasChip)
        if HasChip then
            ...
            setVehData(veh, data)
            TriggerEvent('swt_notifications:Infos','Tjoenertjip v1.05: Vehicle changed!')
...
RegisterNUICallback('reset', function(data)
    ...
    local veh = GetVehiclePedIsUsing(ped)
    resetVeh(veh)
    TriggerEvent('swt_notifications:Infos','Tjoenertjip v1.05: Vehicle is resetted!')
end)
...
AddEventHandler('qb-tunerchip:client:openChip', function()
    ...
    if inVehicle then
        QBCore.Functions.Progressbar("connect_laptop", "Tunerlaptop is getting connected..", 2000, false, true, {
            ...
            StopAnimTask(GetPlayerPed(-1), "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 1.0)
            TriggerEvent("swt_notifications:Infos","Geannuleerd..")
        end)
    else
