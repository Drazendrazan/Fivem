...
-- 						ProcessWeed()
-- 					else
-- 						TriggerEvent('swt_notifications:Infos','You do not have enough Cannabis to process..')
-- 					end
-- 				end, 'cannabis')
-- 			end
-- 		else
			...
			if IsControlJustReleased(0, 38) and not isProcessing then
				QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
					if result then
						...
						TriggerEvent('swt_notifications:Infos','You dont have enough Marijuana')
					end
				end, 'marijuana')
			end
		else
			...
			if IsControlJustReleased(0, 38) and not isProcessing then
				QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
					if result then
						...
						TriggerEvent('swt_notifications:Infos','You dont have enough Cannabis')
					end
				end, 'cannabis')
			end
		else
...
function SpawnWeedPlants()
	while spawnedWeeds < 151 do
		...
		QBCore.Functions.SpawnLocalObject('prop_weed_02', weedCoords, function(obj)
			...
			print("weed spawned dumass go pick it up")
