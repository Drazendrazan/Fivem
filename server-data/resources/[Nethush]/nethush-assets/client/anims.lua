...
RegisterNetEvent('nethush-assets:client:get:tackeled')
AddEventHandler('nethush-assets:client:get:tackeled', function()
 SetPedToRagdoll(PlayerPedId(), math.random(6000, 8000), math.random(6000, 8000), 0, 0, 0, 0) 
end)

function TackleAnim()
 if not QBCore.Functions.GetPlayerData().metadata["ishandcuffed"] and not IsPedRagdoll(PlayerPedId()) then
