	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if not isPickingUp then
				QBCore.Functions.Draw2DText(0.5, 0.88, 'Press ~g~[E]~w~ to pickup weedak47seed', 0.5)
			end
			...
			if IsControlJustReleased(0, 38) and not isPickingUp then
				...
				QBCore.Functions.Progressbar("search_register", "Picking up weed_ak47_seed..", 7500, false, true, {
					...
					TriggerServerEvent('xd_drugs_weed:pickedUpweed_ak47_seed')
				end, function()
					ClearPedTasks(GetPlayerPed(-1))
				end) -- Cancel
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.WeedProcessing.coords, true) < 1 then
			...
			if IsControlJustReleased(0, 38) and not isProcessing then
				...
				if (hasWeed and hasBag) then
					Processweed()
				elseif (hasWeed) then
					TriggerEvent('swt_notifications:Infos','You dont have enough plastic bags.')
				elseif (hasBag) then
					TriggerEvent('swt_notifications:Infos','You dont have enough weed_ak47_seed.')
				else
					TriggerEvent('swt_notifications:Infos','You dont have enough weed_ak47_seed or plastic bags.')
				end
			end
		else
			Citizen.Wait(500)
