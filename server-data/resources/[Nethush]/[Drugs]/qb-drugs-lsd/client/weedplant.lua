	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if not isPickingUp then
				QBCore.Functions.Draw2DText(0.5, 0.88, 'Press ~g~[E]~w~ to pickup acid', 0.5)
			end
			local hasBagd = false
			local s1d = false
	...
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.WeedProcessing.coords, true) < 1 then
			...
			if IsControlJustReleased(0, 38) and not isProcessing then
				...
				elseif (hasBag) then
					QBCore.Functions.Notify('You dont have enough Bottle acid.', 'error')
				else
					QBCore.Functions.Notify('You dont have enough Bottle acid or plastic bags.', 'error')
				end
			end
		else
			Citizen.Wait(500)
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.DrugDealer.coords, true) < 1 then
			...
			if IsControlJustReleased(0, 38) and not isProcessing2 then
				...
				elseif (hasBag2) then
					...
					QBCore.Functions.Notify('You dont have elsd.', 'error')
				end
			end
		else
			Citizen.Wait(500)
