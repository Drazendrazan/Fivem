...
AddEventHandler('xd_drugs_weed:pickedUpCannabis2d', function()
	...
	local Player = QBCore.Functions.GetPlayer(src)
	    ...
	    if 	TriggerClientEvent("QBCore:Notify", src, "Picked up some coke!!", "Success", 8000) then
		  Player.Functions.AddItem('bottle_acid_on', 1) ---- change this shit 
		  Player.Functions.RemoveItem('bottle_acid', 1)----change this
		  TriggerClientEvent("inventory:client:ItemBox", source, QBCore.Shared.Items['bottle_acid_on'], "add")
		  TriggerClientEvent("inventory:client:ItemBox", source, QBCore.Shared.Items['bottle_acid'], "remove")
	    end
end)
...
AddEventHandler('xd_drugs_weed:processweed2d', function()
	...
	if Player.Functions.GetItemByName('bottle_acid_on') and Player.Functions.GetItemByName('empty_plastic_bag') then
		...
		if chance == 1 or chance == 2 or chance == 3 or chance == 4 or chance == 5 or chance == 6 or chance == 7 or chance == 8 then
			...
			Player.Functions.RemoveItem('empty_plastic_bag', 1)----change this
			Player.Functions.AddItem('lsd', 1)-----change this
			TriggerClientEvent("inventory:client:ItemBox", source, QBCore.Shared.Items['bottle_acid_on'], "remove")
			TriggerClientEvent("inventory:client:ItemBox", source, QBCore.Shared.Items['empty_plastic_bag'], "remove")
			TriggerClientEvent("inventory:client:ItemBox", source, QBCore.Shared.Items['lsd'], "add")
			TriggerClientEvent('QBCore:Notify', src, 'LSD Processed successfully', "success")  
		else
