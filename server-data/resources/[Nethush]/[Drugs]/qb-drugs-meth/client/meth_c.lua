	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if IsControlJustReleased(0, 38) and not isPickingUp then
				...
				QBCore.Functions.Progressbar("search_register", "Picking up Acid..", 7500, false, true, {
				...
				QBCore.Functions.Notify('You dont have enough empity Bottle .', 'error')
			end	
			end
		else
			Citizen.Wait(500)
	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if IsControlJustReleased(0, 38) and not isPickingUp then
				...
				--PROP_HUMAN_BUM_BIN animazione
				--prop_cs_cardbox_01 oggetto di spawn  prop_plant_01a
				QBCore.Functions.Progressbar("search_register", "Picking Sulfuric..", 7500, false, true, {
					disableMovement = true,
					disableCarMovement = true,
					disableMouse = false,
					disableCombat = true,
...
--prop_barrel_01a  prop_plant_01a
		QBCore.Functions.SpawnLocalObject('prop_barrel_exp_01a', weedCoords, function(obj) --- change this prop to whatever plant you are trying to use 
			PlaceObjectOnGroundProperly(obj)
			FreezeEntityPosition(obj, true)
...
function Processweed3d()  -- simple animations to loop while process is taking place
	...
	QBCore.Functions.Progressbar("search_register", "Trying to Process..", 12000, false, true, {
		disableMovement = true,
		disableCarMovement = true,
                        disableMouse = false,
                        disableCombat = true,
                                        ...
                                        Citizen.Wait(100)
                                                  ...
                                                  ----se ho l'item eseguo
                                                    if (hasBagd7) then
                                                      ...
                                                      QBCore.Functions.Notify('you don\'t have empty_weed_bag', 'error')
                                                    end
...
function procOn()
    ...
    ----
    if(count == 0) then
    QBCore.Functions.Progressbar("search_register", "Pikup sodium hydroxide", 20000, false, true, {disableMovement = true,
                        disableCarMovement = true,
                        disableMouse = false,
                        disableCombat = true,
                        disableInventory = true,
