...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
    QBCore.Functions.TriggerCallback('xd_drugs:server:RequestConfig', function(DealerConfig)
                                ...
                                if dealer["name"] == "Mystery man" then
                                    ...
                                    if player ~= -1 and distance < 5.0 then
                                        ...
                                        QBCore.Functions.Progressbar("hospital_revive", "Put person back on their feet..", 5000, false, true, {
                                            ...
                                            TriggerEvent("swt_notifications:Infos","You have helped a person!")
                                        ...
                                        TriggerEvent("swt_notifications:Infos","There is no one nearby..")
                                    end
                                else
...
function buyDealerStuff()
    ...
    TriggerServerEvent("nethush-inventory:server:OpenInventory", "shop", "Dealer_"..Config.Dealers[currentDealer]["name"], repItems)
end
...
function knockDoorAnim(home)
    ...
    if home then
        ...
        knockingDoor = false
        Citizen.Wait(1000)
        TriggerEvent('swt_notifications:Infos','It looks like no one is home..')
    end
end

RegisterNetEvent('xd_drugs:client:updateDealerItems')
AddEventHandler('xd_drugs:client:updateDealerItems', function(itemData, amount)
    TriggerServerEvent('xd_drugs:server:updateDealerItems', itemData, amount, currentDealer)
end)

RegisterNetEvent('xd_drugs:client:setDealerItems')
AddEventHandler('xd_drugs:client:setDealerItems', function(itemData, amount, dealer)
...
function requestDelivery()
    ...
    TriggerServerEvent('xd_drugs:server:giveDeliveryItems', amount)
    SetTimeout(2000, function()
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            ...
            button = {
                enabled = true,
                buttonEvent = "xd_drugs:client:setLocation",
...
function setMapBlip(x, y)
    SetNewWaypoint(x, y)
    TriggerEvent('swt_notifications:Infos','The route to the delivery location has been set on your map.');
end

RegisterNetEvent('xd_drugs:client:setLocation')
AddEventHandler('xd_drugs:client:setLocation', function(locationData)
    if activeDelivery == nil then
        activeDelivery = locationData
    else
        setMapBlip(activeDelivery["coords"]["x"], activeDelivery["coords"]["y"])
        TriggerEvent('swt_notifications:Infos','You still have an open delivery...')
        return
    end
...
function deliverStuff(activeDelivery)
    if deliveryTimeout > 0 then
        ...
        QBCore.Functions.Progressbar("work_dropbox", "Delivering products..", 3500, false, true, {
            ...
            TriggerServerEvent('xd_drugs:server:succesDelivery', activeDelivery, true)
        end, function() -- Cancel
            ClearPedTasks(GetPlayerPed(-1))
            TriggerEvent("swt_notifications:Infos","Canceled..")
        end)
    else
        TriggerServerEvent('xd_drugs:server:succesDelivery', activeDelivery, false)
    end
    deliveryTimeout = 0
end
...
function doPoliceAlert()
    ...
    TriggerServerEvent('xd_drugs:server:callCops', streetLabel, pos)
end

RegisterNetEvent('xd_drugs:client:robberyCall')
AddEventHandler('xd_drugs:client:robberyCall', function(msg, streetLabel, coords)
...
RegisterNetEvent('xd_drugs:client:sendDeliveryMail')
AddEventHandler('xd_drugs:client:sendDeliveryMail', function(type, deliveryData)
...
RegisterNetEvent('xd_drugs:client:CreateDealer')
AddEventHandler('xd_drugs:client:CreateDealer', function(dealerName, minTime, maxTime)
    local ped = GetPlayerPed(-1)
    ...
    TriggerServerEvent('xd_drugs:server:CreateDealer', DealerData)
end)

RegisterNetEvent('xd_drugs:client:RefreshDealers')
AddEventHandler('xd_drugs:client:RefreshDealers', function(DealerData)
    Config.Dealers = DealerData
end)

RegisterNetEvent('xd_drugs:client:GotoDealer')
AddEventHandler('xd_drugs:client:GotoDealer', function(DealerData)
    local ped = GetPlayerPed(-1)

    SetEntityCoords(ped, DealerData["coords"]["x"], DealerData["coords"]["y"], DealerData["coords"]["z"])
    TriggerEvent('swt_notifications:Infos','You have been teleported : '.. DealerData["name"] .. ' Good luck!')
end)