...
RegisterNetEvent('xd_drugs:client:cornerselling')
AddEventHandler('xd_drugs:client:cornerselling', function(data)
    QBCore.Functions.TriggerCallback('xd_drugs:server:cornerselling:getAvailableDrugs', function(result)
        if result ~= nil then
            ...
            for _, zone in pairs(Config.CornerSellingZones) do
                ...
                --if zoneDist < 200 then
                    if not cornerselling then
                        cornerselling = true
                        TriggerEvent('swt_notifications:Infos','Corner selling mode enabled..')
                        startLocation = GetEntityCoords(GetPlayerPed(-1))
                        -- TaskStartScenarioInPlace(GetPlayerPed(-1), "CODE_HUMAN_CROSS_ROAD_WAIT", 0, false)
                    --else
                    --    cornerselling = false
                    --    TriggerEvent('swt_notifications:Infos','Corner selling position disabled.')
                        -- ClearPedTasks(GetPlayerPed(-1))
                    end
               -- else
                --    TriggerEvent('swt_notifications:Infos','You are too away from allowed selling zones.','error')
               -- end
            end
        else
            TriggerEvent('swt_notifications:Infos','You dont have drugs on you..')
        end
    end)
end)
RegisterNetEvent('xd_drugs:client:cornersellingstop')
AddEventHandler('xd_drugs:client:cornersellingstop', function(data)
...
function toFarAway()
    TriggerEvent('swt_notifications:Infos','You walked away. Failed')
    cornerselling = false
    hasTarget = false
    busySelling = false
    startLocation = nil
            ...
            if IsEntityDead(stealingPed) then
                ...
                if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, pedpos.x, pedpos.y, pedpos.z, true) < 1.5) then
                    ...
                    if IsControlJustReleased(0, Keys["E"]) then
                        ...
                        TriggerEvent('nethush-inventory:client:ItemBox', QBCore.Shared.Items[stealData.item], "add")
                        stealingPed = nil
                        stealData = {}
                    end
                end
...
RegisterNetEvent('xd_drugs:client:refreshAvailableDrugs')
AddEventHandler('xd_drugs:client:refreshAvailableDrugs', function(items)
...
function SellToPed(ped)
    ...
    if hasTarget then
        while pedDist < 1.5 do
            ...
            if getRobbed == 18 or getRobbed == 9 then
                TriggerServerEvent('xd_drugs:server:robCornerDrugs', availableDrugs[drugType].item, bagAmount)
                TriggerEvent('swt_notifications:Infos','You have been robbed of - $'..bagAmount..' bag(\'s) '..availableDrugs[drugType].label)
                stealingPed = ped
                stealData = {
                ...
                if pedDist < 1.5 then
                    ...
                    if IsControlJustPressed(0, Keys["E"]) then
                        TriggerEvent('swt_notifications:Infos','Offer accepted!')
                        TriggerServerEvent('xd_drugs:server:sellCornerDrugs', availableDrugs[drugType].item, bagAmount, randomPrice)
                        hasTarget = false
                    ...
                    if IsControlJustPressed(0, Keys["G"]) then
                        TriggerEvent('swt_notifications:Infos','Declined!')
                        hasTarget = false
