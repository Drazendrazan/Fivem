...
AddEventHandler('vehiclekeys:client:SetOwner', function(plate)
    ...
    HasKey = true
      TriggerEvent("swt_notifications:Success","SLMC System",'You picked the keys of the vehicle',"top-right",3500,true)
            ...
            if result then
                if HasKey then
                    ...
                    if vehLockStatus == 1 then
                        ...
                        TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 5, "lock", 0.10)
                        SetVehicleDoorsLocked(veh, 2)
                        if(GetVehicleDoorLockStatus(veh) == 2)then
                          TriggerEvent("swt_notifications:Success","SLMC System",'Vehicle locked!',"top-right",8000,true)

                        else
                            TriggerEvent("swt_notifications:Warning","SLMC System",'Something went wrong whit the locking system!',"top-right",3000,true)
                        end
                    else
                        Citizen.Wait(750)
                        ClearPedTasks(GetPlayerPed(-1))
                        ...
                        if(GetVehicleDoorLockStatus(veh) == 1)then
                            TriggerEvent("swt_notifications:Success","SLMC System",'Vehicle Unlocked!',"top-right",8000,true)
                ...
                TriggerEvent("swt_notifications:Warning","SLMC System",'You dont have the keys of the vehicle..',"top-right",3000,true)
...
function LockpickDoor(isAdvanced)
    ...
    if vehicle ~= nil and vehicle ~= 0 then
        ...
        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 1.5 then
            ...
            if (vehLockStatus > 1) then
                local lockpickTime = math.random(25000, 45000)
                if isAdvanced then
                    lockpickTime = math.ceil(lockpickTime*0.25)
                end
                ...
                QBCore.Functions.Progressbar("lockpick_vehicledoor", "breaking the door open..", lockpickTime, false, true, {
                    ...
                    if math.random(1, 100) <= 90 then
                        TriggerEvent("swt_notifications:Success","SLMC System",'Door open!',"top-right",9000,true)
                        TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 5, "unlock", 0.3)
                        SetVehicleDoorsLocked(vehicle, 0)
                        SetVehicleDoorsLockedForAllPlayers(vehicle, false)
                    else
                        TriggerEvent("swt_notifications:Warning","SLMC System",'Failed!',"top-right",3000,true)
                ...
                Skillbar.Start({
                    ...
                    if IsHotwiring then
                        if SucceededAttempts + 1 >= NeededAttempts then
                            ...
                            TriggerEvent("swt_notifications:Success","SLMC System",'Lockpicking succeeded!',"top-right",3000,true)
                            HasKey = true
                            TriggerEvent("vehiclekeys:client:SetOwner", GetVehicleNumberPlateText(vehicle))
                            IsHotwiring = false
                            FailedAttemps = 0
                            ...
                            if vehicle ~= nil and vehicle ~= 0 then
                                ...
                                TriggerEvent("swt_notifications:Warning","SLMC System",'You have to be in the vehicle',"top-right",9000,true)
                                IsHotwiring = false
                    ...
                    if IsHotwiring then
                        ...
                        HasKey = false
                        SetVehicleEngineOn(vehicle, false, false, true)
                        TriggerEvent("swt_notifications:Warning","SLMC System",'Lockpicking failed!',"top-right",9000,true)
                        IsHotwiring = false
                        FailedAttemps = FailedAttemps + 1
                        local c = math.random(2)
                        local o = math.random(2)
...
function nethushlockpick()
        local vehicle = GetVehiclePedIsIn(GetPlayerPed(-1), true)
        local hotwireTime = math.random(5000, 10000)
        local time = math.random(7,15)
    ...
    if not HasKey then 
        ...
        QBCore.Functions.Progressbar("hotwire_vehicle", "Engaging the ignition switch", hotwireTime, false, true, {
            ...
            if (math.random(0, 100) < 10) then
                HasKey = true
                TriggerEvent("swt_notifications:Success","SLMC System",'Hotwire succeeded!',"top-right",7000,true)

            else
                HasKey = false
                SetVehicleEngineOn(veh, false, false, true)
                TriggerEvent("swt_notifications:Warning","SLMC System",'Hotwire failed!',"top-right",9000,true)
            end
            IsHotwiring = false
        end, function() -- Cancel
            StopAnimTask(GetPlayerPed(-1), "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 1.0)
