...
AddEventHandler('vehiclekeys:server:GiveVehicleKeys', function(plate, target)
    ...
    if CheckOwner(plate, Player.PlayerData.citizenid) then
        if QBCore.Functions.GetPlayer(target) ~= nil then
            ...
            TriggerClientEvent("swt_notifications:Info",src,"SLMC System","You gave the keys!","top-right",5500,true)
            TriggerClientEvent("swt_notifications:Info",target,"SLMC System","You got the keys!","top-right",5500,true)
        else
...
QBCore.Commands.Add("givekeys", "Give keys of the vehicle", {{name = "id", help = "Speler id"}}, true, function(source, args)
	local src = source
