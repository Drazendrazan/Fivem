...
RegisterNetEvent('nethush-nopixelmenu:client:flip:vehicle')
AddEventHandler('nethush-nopixelmenu:client:flip:vehicle', function()
    ...
    if Vehicle ~= 0 and Distance < 1.7 then
        QBCore.Functions.Progressbar("flip-vehicle", "Flippin the freaking car..", math.random(10000, 15000), false, true, {
            ...
            flags = 49,
             ...
             TriggerEvent("swt_notifications:Infos","Succes")
        end, function()
            TriggerEvent("swt_notifications:Infos","Cancelled..")
        end)
    else
        TriggerEvent("swt_notifications:Infos","No vehicle found..")
    end
end)

RegisterNetEvent('nethush-nopixelmenu:client:setExtra')
AddEventHandler('nethush-nopixelmenu:client:setExtra', function(data)
        ...
        if GetPedInVehicleSeat(veh, -1) == PlayerPedId() then
            if DoesExtraExist(veh, extra) then 
                if IsVehicleExtraTurnedOn(veh, extra) then
                    ...
                    SetVehicleEngineHealth(veh, enginehealth)
                    SetVehicleBodyHealth(veh, bodydamage)
                    TriggerEvent('swt_notifications:Infos','Extra ' .. extra .. ' turned on')
                end    
            else
                TriggerEvent('swt_notifications:Infos','Extra ' .. extra .. ' does not exist on this car')
            end
        else
            TriggerEvent('swt_notifications:Infos','Your not a driver for this vehicle!')
        end
    end
end)

RegisterNetEvent("nethush-nopixelmenu:client:send:panic:button")
AddEventHandler("nethush-nopixelmenu:client:send:panic:button",function()
...
RegisterNetEvent("nethush-nopixelmenu:client:send:down")
AddEventHandler("nethush-nopixelmenu:client:send:down",function(Type)
    local Player = QBCore.Functions.GetPlayerData()
    local Info = {['Firstname'] = Player.charinfo.firstname, ['Lastname'] = Player.charinfo.lastname, ['Callsign'] = Player.metadata['callsign']}
    local StreetLabel = QBCore.Functions.GetStreetLabel()
    local Priority = 2
...
RegisterNetEvent("nethush-nopixelmenu:client:open:door")
AddEventHandler("nethush-nopixelmenu:client:open:door",function(DoorNumber)
