...
AddEventHandler('qb-trunk:client:KidnapTrunk', function()
    ...
    if (distance ~= -1 and distance < 2) then
        if isKidnapping then
           ...
           -- TriggerEvent('swt_notifications:Infos','You have not kidnapped this person!')
            TriggerEvent("swt_notifications:Negative","SLMC System",'You have not kidnapped this person!',"top-right",3000,true)
                    ...
                    if not isBusy then
                        if not isKidnapped then
                            -- if GetVehicleDoorAngleRatio(closestVehicle, 5) > 0 then
                               ...
                               -- TriggerEvent('swt_notifications:Infos','Youre in the trunk.', 'success', 4000)
                                TriggerEvent("swt_notifications:Success","SLMC System",'Youre in the trunk.',"top-right",4000,true)
                                TrunkCam(true)

                                isKidnapped = true
                            -- else
                                -- TriggerEvent('swt_notifications:Infos','Is de kofferbak dicht?')
                            -- end
                        else
                            local ped = GetPlayerPed(-1)
                            local vehicle = GetEntityAttachedTo(PlayerPedId())
                            ...
                            if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                               ...
                               -- TriggerEvent('swt_notifications:Infos','Is the trunk closed??')
                                TriggerEvent("swt_notifications:Warning","SLMC System",'Is the trunk closed??',"top-right",2500,true)

                            end
                        end
                    else
                      -- TriggerEvent('swt_notifications:Infos','Does anyone already see it?')
                        TriggerEvent("swt_notifications:Warning","SLMC System",'Does anyone already see it?',"top-right",2500,true)
                    end 
                else
                 --   TriggerEvent('swt_notifications:Infos','Youre already in the trunk')
                 TriggerEvent("swt_notifications:Warning","SLMC System",'Youre already in the trunk',"top-right",2500,true)
                end 
            else
              --  TriggerEvent('swt_notifications:Infos','You cannot put this vehicle in the trunk..')
              TriggerEvent("swt_notifications:Warning","SLMC System",'You cannot put this vehicle in the trunk..',"top-right",2500,true)
            end
        end, plate)
    else
       -- TriggerEvent('swt_notifications:Infos','You cannot put this vehicle in the trunk..')
...
AddEventHandler('qb-trunk:client:GetIn', function(isKidnapped)
    ...
    if closestVehicle ~= 0 then
        ...
        --TriggerEvent('swt_notifications:Infos','No vehicle in sight..')
        TriggerEvent("swt_notifications:Warning","SLMC System",'No vehicle in sight..',"top-right",2500,true)
    end
end)
        ...
        if inTrunk then
            if not isKidnapped then
                ...
                if DoesEntityExist(vehicle) then
                    ...
                    if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                        DrawText3Ds(drawPos.x, drawPos.y, drawPos.z + 0.5, '[G] Trunk to close')
                        if IsControlJustPressed(0, Keys["G"]) then
                            if not IsVehicleSeatFree(vehicle, -1) then
                                TriggerServerEvent('nethush-radialmenu:trunk:server:Door', false, plate, 5)
                            else
                                SetVehicleDoorShut(vehicle, 5, false)
                            end
                        end
                    else
                        DrawText3Ds(drawPos.x, drawPos.y, drawPos.z + 0.5, '[G] Trunk to open')
                        if IsControlJustPressed(0, Keys["G"]) then
                            if not IsVehicleSeatFree(vehicle, -1) then
                                TriggerServerEvent('nethush-radialmenu:trunk:server:Door', true, plate, 5)
                            else
