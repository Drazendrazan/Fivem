$(document).on('click', '.menu-item', function(e) {
    ...
    if ($(this).data('event') != null && $(this).data('event') != undefined) {
        $.post('http://nethush-eye/DoSomething', JSON.stringify({event: $(this).data('event'), type: $(this).data('eventtype'), parameter: $(this).data('eventpara')}));
        setTimeout(function(){
            CloseEye()
        }, 50);
    }
...
CloseEye = function() {
    ...
    setTimeout(function(){
        ...
        $.post(`http://nethush-eye/CloseUi`, JSON.stringify({}));
    }, 15);
}
...
window.addEventListener("mousedown", function onEvent(event) {
    if (event.button == 2) {
        CloseMenu()
        $.post(`http://nethush-eye/CloseMenu`, JSON.stringify({}));
    }
});