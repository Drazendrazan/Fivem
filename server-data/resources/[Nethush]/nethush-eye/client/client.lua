            ...
            if not Config.TrunkData[Plate]['Busy'] then
                if GetVehicleDoorAngleRatio(Vehicle, 5) > 0 then
                    ...
                    if ClassData['CanEnter'] then
                        InTrunk = true
                        exports['nethush-assets']:RequestAnimationDict("fin_ext_p1-7")
                        ...
                        TriggerEvent('swt_notifications:Infos','Ever hopped in a trunk of a motorcycle?')
                    end
                else
                    TriggerEvent('swt_notifications:Infos','Is the trunk even open?')
                end
            else
                TriggerEvent('swt_notifications:Infos','There already is someone in the trunk.')
            end
        else
            if not Config.TrunkData[Plate]['Busy'] then
                if GetVehicleDoorAngleRatio(Vehicle, 5) > 0 then
...
AddEventHandler('qb-eye:client:getout:trunk', function()
    ...
    if GetVehicleDoorAngleRatio(Vehicle, 5) > 0 then
        ...
        TriggerEvent('swt_notifications:Infos','Is the trunk open?')
    end
end)
