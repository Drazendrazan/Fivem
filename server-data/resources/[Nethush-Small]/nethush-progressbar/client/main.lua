...
function Process(action, start, tick, finish)
    ...
    if not IsEntityDead(PlayerPedId()) or Action.useWhileDead then
        if not isDoingAction then
            ...
            TriggerEvent("swt_notifications:Infos","You are already doing something")
        end
    else
        TriggerEvent("swt_notifications:Infos","Can not perform action")
    end
end
                    ...
                    if Action.animation ~= nil then
                        ...
                        elseif Action.animation.animDict ~= nil and Action.animation.anim ~= nil then
                            ...
                            if (DoesEntityExist(player) and not IsEntityDead(player)) then
                                exports['nethush-assets']:RequestAnimationDict(Action.animation.animDict)
...
function Process(action, start, tick, finish)
    ...
    if not IsEntityDead(PlayerPedId()) or Action.useWhileDead then
        if not isDoingAction then
            ...
            TriggerEvent("swt_notifications:Infos","You are already working on something...")
        end
    else
        TriggerEvent("swt_notifications:Infos","Something went wrong!")
    end
end
