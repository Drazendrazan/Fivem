...
PoliceGarage = function(type)
	...
	QBCore.UI.Menu.Open('default', GetCurrentResourceName(), "esx_policeGarage_menu",
		{
	...
	function(data, menu)
		...
		elseif data.current.action == 'store_vehicle' then
			local veh,dist = QBCore.Functions.GetClosestVehicle(playerCoords)
			if dist < 3 then
				DeleteEntity(veh)

				TriggerEvent("swt_notifications:Infos",Config.VehicleParked)
			else
				TriggerEvent("swt_notifications:Infos",Config.NoVehicleNearby)
			end
			insideMarker = false
		end
...
VehicleMenu = function(type)
	...
	QBCore.UI.Menu.Open('default', GetCurrentResourceName(), "esx_policeGarage_vehicle_garage",
		{
			title    = Config.TitlePoliceGarage,
			align    = "center",
			elements = elements
	...
	function(data, menu)
		...
		SetVehicleNumberPlateText(veh, "POLICE"..tostring(math.random(1200, 9999)))
	...
	TriggerEvent("vehiclekeys:client:SetOwner", plaquevehicule)
		...
		if type == 'car' then
			TriggerEvent("swt_notifications:Infos",Config.CarOutFromPolGar)
		elseif type == 'helicopter' then
			TriggerEvent("swt_notifications:Infos",Config.HeliOutFromPolGar)
		elseif type == 'boat' then
			TriggerEvent("swt_notifications:Infos",Config.BoatOutFromPolGar)
		end
		...
		for i = 1, #v.Pos, 1 do
			if IsPedInAnyVehicle(ped, true) then
				...
				if GetDistanceBetweenCoords(GetEntityCoords(ped), v.Pos[i].x, v.Pos[i].y, v.Pos[i].z, true) <= Config.Distance then
					TriggerEvent("swt_notifications:Infos",Config.VehRepNotify)
					FreezeEntityPosition(veh, true)
				--	exports['progressBars']:startUI((Config.RepairTime * 1000), Config.Progress1)
					Citizen.Wait((Config.RepairTime * 1000))
					TriggerEvent("swt_notifications:Infos",Config.VehRepDoneNotify)
					SetVehicleFixed(veh)
	...
	for k,v in pairs(Config.CarZones) do
		for i = 1, #v.Pos, 1 do
			if IsPedInAnyVehicle(ped, true) then
				local veh = GetVehiclePedIsIn(ped, false)
				if GetDistanceBetweenCoords(GetEntityCoords(ped), v.Pos[i].x, v.Pos[i].y, v.Pos[i].z, true) <= Config.Distance then
					TriggerEvent("swt_notifications:Infos",Config.VehCleanNotify)
					FreezeEntityPosition(veh, true)
				--	exports['progressBars']:startUI((Config.CleanTime * 1000), Config.Progress2)
					Citizen.Wait((Config.CleanTime * 1000))
					TriggerEvent("swt_notifications:Infos",Config.VehCleanDoneNotify)
					SetVehicleDirtLevel(veh, 0.1)
					FreezeEntityPosition(veh, false)
				end
			end
...
function OpenExtraMenu()
	...
	QBCore.UI.Menu.Open('default', GetCurrentResourceName(), 'extra_actions', {
...
function OpenLiveryMenu()
	...
	QBCore.UI.Menu.Open('default', GetCurrentResourceName(), 'livery_menu', {
...
function OpenMainMenu()
	...
	QBCore.UI.Menu.Open('default', GetCurrentResourceName(), 'color_menu', {
		title    = Config.TitlePoliceExtra,
		align    = 'top-left',
		elements = elements
	}, function(data, menu)
...
function OpenMainColorMenu(colortype)
	...
	QBCore.UI.Menu.Open('default', GetCurrentResourceName(), 'main_color_menu', {
