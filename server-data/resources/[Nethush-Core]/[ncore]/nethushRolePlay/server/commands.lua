...
QBCore.Commands.Add("setmoney", "set a players money amount", {{name="id", help="Player ID"},{name="moneytype", help="Type of money (cash, bank, crypto)"}, {name="amount", help="Amount of money"}}, true, function(source, args)
	...
	if Player ~= nil then
		...
		TriggerClientEvent('chatMessage', source, "SYSTEM", "error", "Player is not online macahn!")
	end
end, "admin")

QBCore.Commands.Add("setjob", "Give a job to a player", {{name="id", help="Player ID"}, {name="job", help="Job name"}, {name="grade", help="level"}}, true, function(source, args)
	local Player = QBCore.Functions.GetPlayer(tonumber(args[1]))
	if Player ~= nil then
		Player.Functions.SetJob(tostring(args[2]), tonumber(args[3]))
	else
		--TriggerClientEvent('swt_notifications:Infos', source, "Player is not online!")
		TriggerClientEvent("swt_notifications:Negative","SLMC System","Player is not online!","top-right",1000,true)
	end
end, "admin")

QBCore.Commands.Add("job", "See what your job is", {}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
	--TriggerClientEvent('swt_notifications:Infos', source, "Job : "..Player.PlayerData.job.label.." - "..Player.PlayerData.job.gradelabel)
	TriggerClientEvent("swt_notifications:Info",source,"SLMC System","Job : "..Player.PlayerData.job.label.." - "..Player.PlayerData.job.gradelabel,"right",8000,true)
...
QBCore.Commands.Add("gang", "See what gang you're in", {}, false, function(source, args)
	...
	if Player.PlayerData.gang.name ~= "geen" then
		...
		--TriggerClientEvent('swt_notifications:Infos', source, "You're not in a gang!")
		TriggerClientEvent("swt_notifications:Info",source,"SLMC System","You're not in a gang!","right",2000,true)
	end
end)
...
--	TriggerClientEvent('swt_notifications:Infos', source, "Cleared Server Chat")
	TriggerClientEvent("swt_notifications:Info",source,"SLMC System","Cleared Server Chat","right",2000,true)
	
end, "admin")

QBCore.Commands.Add("testnotify", "test notify", {{name="text", help="Tekst enzo"}}, true, function(source, args)
	TriggerClientEvent('swt_notifications:Infos', source, table.concat(args, " "))
end, "god")
