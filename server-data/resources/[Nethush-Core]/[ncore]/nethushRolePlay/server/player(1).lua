...
QBCore.Player.CreatePlayer = function(PlayerData)
	...
	self.Functions.SetJob = function(job, grade)
		...
		if QBCore.Shared.Jobs[job] ~= nil then
			...
			self.PlayerData.job.gradelabel = QBCore.Shared.Jobs[job].grades[grade].label
	...
	self.Functions.AddMoney = function(moneytype, amount, reason)
		...
		if self.PlayerData.money[moneytype] ~= nil then
			...
			if amount > 100000 then
				TriggerEvent("qb-log:server:CreateLog", "playermoney", "AddMoney", "lightgreen", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** LKR"..amount .. " ("..moneytype..") added, new "..moneytype.." balance: "..self.PlayerData.money[moneytype], true)
			else
				TriggerEvent("qb-log:server:CreateLog", "playermoney", "AddMoney", "lightgreen", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** LKR"..amount .. " ("..moneytype..") added, new "..moneytype.." balance: "..self.PlayerData.money[moneytype])
			end
			TriggerClientEvent("hud:client:OnMoneyChange", self.PlayerData.source, moneytype, amount, false)
			TriggerClientEvent('qb-phone_new:client:addBankMoney', self.PlayerData.source, amount)
			return true
		end
		return false
	end
	...
	self.Functions.AddItem = function(item, amount, slot, info)
		...
		if (totalWeight + (itemInfo["weight"] * amount)) <= QBCore.Config.Player.MaxWeight then
			if (slot ~= nil and self.PlayerData.items[slot] ~= nil) and (self.PlayerData.items[slot].name:lower() == item:lower()) and (itemInfo["type"] == "item" and not itemInfo["unique"]) then
				...
				--TriggerClientEvent('swt_notifications:Infos', self.PlayerData.source, itemInfo["label"].. " toegevoegd!")
				return true
	...
	self.Functions.RemoveItem = function(item, amount, slot)
		...
		if slot ~= nil then
			if self.PlayerData.items[slot].amount > amount then
				...
				--TriggerClientEvent('swt_notifications:Infos', self.PlayerData.source, itemInfo["label"].. " verwijderd!")
				return true
			else
