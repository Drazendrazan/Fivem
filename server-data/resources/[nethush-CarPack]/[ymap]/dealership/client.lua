...
RegisterNUICallback('importVehicle', function(data, cb)
	if cooldown == nil then
		...
		if truck then
			...
			TriggerEvent("swt_notifications:Infos","You already have a delivery in progress")
			...
			if checkPos == false then
				...
				TriggerEvent("swt_notifications:Infos","Garage is blocked")
...
AddEventHandler('lc_dealership:startContract', function(vehicle,amount,isexport,request_id)
	...
	TriggerEvent("swt_notifications:Infos","The truck has been successfully picked up, let's get in the car and attach the box to get started")
	...
	while truck do
		...
		if IsEntityDead(ped) then
			...
			TriggerEvent("swt_notifications:Infos","You died and failed")
		...
		if engineH <= 150 then
			...
			TriggerEvent("swt_notifications:Infos","Your vehicle has been destroyed")
